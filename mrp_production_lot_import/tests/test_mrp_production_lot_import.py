...
class TestMrpProductionLotImport(TransactionCase):
    ...
    def test_import_lot(self):
        ...
        mo.action_assign()
        lots = mo.move_raw_ids.mapped('active_move_line_ids.lot_id')
    ...
    def test_duplicate_import_lot(self):
        ...
        self.assertEqual(
            ...
            'Please check the following serial number (name, id):'
            ' \n(TABLEBOARD001, %s)' % self.lot_1.id, result.exception.name)

        mo_2.move_raw_ids[0].active_move_line_ids[0].lot_id = self.lot_3
        mo_2.move_raw_ids[0].active_move_line_ids[1].lot_id = self.lot_4
        with self.assertRaises(ValidationError) as result:
            mo_2.button_mark_done()
        self.assertEqual(
            'A serial number should only be linked to a single product.\n'
            'Please check the following serial number (name, id):'
            ' \n(TABLE001, %s)' % lots[0].id, result.exception.name)
        leg_quant = self.env['stock.quant'].search([
            ('product_id', '=', self.product_table_leg.id),
            ('location_id', '=', self.stock_location.id),
        ])
        ...
        self.assertEqual(leg_quant.quantity, 0)
        self.assertEqual(sum(board_quant.mapped('quantity')), -1)
        self.assertEqual(sum(table_quant.mapped('quantity')), 2)
    ...
    def test_import_two_tracking_material(self):
        ...
        self.assertEqual(
            ...
            % self.product_table_leg.display_name, result.exception.name)
