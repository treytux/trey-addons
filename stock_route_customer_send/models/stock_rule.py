...
class StockRule(models.Model):
    ...
    def _prepare_purchase_order(
        ...
        if not route_ids or not group_id:
            return res
        if not route_ids.is_customer_send:
            return res
        if route_ids.is_customer_send:
            res.update({
                'sale_id': group_id.sale_id.id,
                'customer_shipping_id': (
                    group_id.sale_id.partner_shipping_id.id),
                'customer_order_ref': (
                    group_id.sale_id.client_order_ref
                    or group_id.sale_id.name),
            })
        return res
