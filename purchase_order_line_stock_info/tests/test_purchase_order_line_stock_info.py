...
class TestPurchaseOrderLineStockInfo(common.TransactionCase):
    ...
    def test_purchase_order_line_stock_info(self):
        qty = 100.0
        ...
        self.assertEqual(
            purchase.order_line[0].product_id.qty_available_real,
            qty - qty_sale)
        self.assertTrue(purchase.order_line[0].stock_info)
        line = purchase.order_line[0]
        stock_info = line.stock_info.split(',')
        self.assertEqual(len(stock_info), 3)
        self.assertEqual('On hand: 100.0', stock_info[0])
        self.assertEqual('Stock real: 70.0', stock_info[1])
        self.assertEqual('Order warehouse: 70.0', stock_info[2])
