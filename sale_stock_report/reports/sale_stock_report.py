...
class SaleStockReport(models.Model):
    ...
    price_subtotal_delivered = fields.Float(
        string='Price subtotal delivered',
        readonly=True,
    )
    ...
    def init(self, cr):
        tools.sql.drop_view_if_exists(cr, 'sale_stock_report')
        cr.execute('''
    ...
    def _from_join(self):
        return '''
            FROM((
                SELECT
                    move.id AS id,
                    l.order_id AS sale_id,
                    so.date_order AS date_order,
                    move.picking_id AS picking_id,
                    l.product_uom_qty AS qty_ordered,
                    ...
                    CASE WHEN move.state = 'done'
                        THEN l.price_unit * move.product_uom_qty * (
                            1 - (l.discount / 100))
                        ELSE 0
                        END AS price_subtotal_delivered,
                    ...
                    move.invoice_state as invoice_state
                FROM procurement_order AS proc
                LEFT JOIN sale_order_line AS l ON l.id = proc.sale_line_id
                LEFT JOIN stock_move AS move ON move.procurement_id = proc.id
                LEFT JOIN stock_location loc ON (move.location_dest_id=loc.id)
                ...
                LEFT JOIN stock_location loc_dst ON (
                    move.location_id=loc_dst.id)
                LEFT JOIN sale_order so ON (l.order_id=so.id)
                WHERE loc_dst.usage = 'customer'
                )
            ) AS foo
        '''
