...
class ProductTemplate(models.Model):
    ...
    def name_get(self):
        ...
        name_pattern = self.env['ir.config_parameter'].sudo().get_param(
            'product_name_get.product_template_name_pattern', default='')
        origin = super(ProductTemplate, self).name_get()
        data = {}
        for product_tmpl in self:
            possible_fields = self.find_possible_fields(name_pattern)
            for poss_field in possible_fields:
                if poss_field not in product_tmpl._fields:
                    _log.error((
                        '[name_get product.template]: the field \'%s\' does '
                        'not found in model.' % poss_field))
                    return origin
                data.update({
                    poss_field: getattr(product_tmpl, poss_field) or '',
                })
            name = name_pattern % data
            result.append((product_tmpl.id, name))
        return result
