...
class TestProductGet(TransactionCase):
    ...
    def test_product_get(self):
        ...
        self.assertEqual(product.name, 'pp.name')
        self.assertEqual(product.product_tmpl_id.name, 'pp.name')
        product_pattern = self.env.ref(
            'product_name_get.product_product_name_pattern')
        product_pattern.value = '%(name)s'
        self.assertEqual(product.name_get()[0][1], 'pp.name')
    ...
    def test_product_get_variants(self):
        ...
        self.assertEqual(variant._name, 'product.product')
        product_pattern = self.env.ref(
            'product_name_get.product_product_name_pattern')
        product_pattern.value = ''
        self.assertEqual(variant.name_get()[0][1], 'pt.name (V1)')
        product_pattern.value = '%(product_tmpl_id.name)s %(name)s'
        self.assertEqual(variant.name_get()[0][1], 'pt.name pt.name')
        product_pattern.value = '%(name)s'
        self.assertEqual(variant.name_get()[0][1], 'pt.name')
