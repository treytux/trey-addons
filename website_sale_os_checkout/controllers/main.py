...
class website_sale(website_sale):
    mandatory_billing_fields = [
        'name', 'email', 'street', 'country_id', 'zip', 'city']
    optional_billing_fields = [
        'street2', 'state_id', 'vat', 'vat_subjected', 'phone', 'mobile']
    ...
    def checkout(self, **post):
        ...
        if request.uid != request.website.user_id.id:
            billing_ids = env['res.partner'].sudo().search([
                ('parent_id', '=', partner.id),
                ('type', '=', 'invoice')])

            # billing_ids = list(set([partner.id]) | set(billing_ids))
            billing_ids = list(
                set([partner.parent_id.id]) | set(billing_ids))

        # ctx = dict(context, show_address=1)
        billings = billing_ids and env['res.partner'].browse(
            list(billing_ids)) or []

        values['billings'] = billings
        if 'company' not in values['checkout']:
            values['checkout'].update({'company': partner.parent_id.name})

        values['mandatory_billing_fields'] = \
            self._get_mandatory_billing_fields()
        values['mandatory_shipping_fields'] = \
            self._get_mandatory_shipping_fields()

        # multi_addresses = request.website.user_id.has_group(
        #     'sale.group_delivery_invoice_address')
        # _log.info('=' * 30)
        # _log.info(('multi_addresses', multi_addresses))
        # _log.info('=' * 30)
        # # values['delivery_addresses']
        return request.website.render(
            'website_sale_os_checkout.osc_onestepcheckout', values)
    ...
    def confirm_address(self, **post):
        ...
        if partner:
            _log.info('=' * 30)
            _log.info(('writeeeeeeeeeeeeeee', 'writeeeeeeeeeeeeeee'))
            _log.info('=' * 30)
            partner.sudo().write(billing_info)
            if partner.parent_id.is_company and 'vat' in billing_info:
                partner.parent_id.sudo().write({'vat': billing_info['vat']})
        else:
            _log.info('=' * 30)
            _log.info(('cccccccccccccccccccreate', 'cccccccccccccccccccreate'))
            _log.info('=' * 30)
    ...
    def checkout_terms(self, **opt):
        ...
        return request.website.render(
            'website_sale_os_checkout.checkout_terms')
