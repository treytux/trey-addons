(function() {
  ...
  function startTransaction(acquirer_id, form) {
    ...
    // Where is defined openerp.website.prePaymenProcess???
        ...
        if (result) {
          if (result.success) {
            if (result.order_total) {
              ...
              $('.o_osc_order .js_payment input[name=amount]').val(result.order_total);
            }
  ...
  function validateAddress(ev) {
    ...
    $('.o_website_sale_os_checkout .has-error').removeClass('has-error');
      ...
      .then(function (result) {
        if (result) {
          if (result.success) {
            ...
            var acquirer_id = $(ev.currentTarget).parents('.oe_sale_acquirer_button').first().data('id');
            if (! acquirer_id) {
              return false;
            }
            ...
            for (var key in result.errors) {
              if ($('.o_website_sale_os_checkout input[name=' + key + ']').length > 0) {
                $('.o_website_sale_os_checkout input[name=' + key + ']').parent().addClass('has-error');
              } else if ($('.o_website_sale_os_checkout select[name=' + key + ']').length > 0) {
                $('.o_website_sale_os_checkout select[name=' + key + ']').parent().addClass('has-error');
              }
            }
          }
        } else {
  ...
  website.dom_ready.done(function () {
    ...
    $carrier.find('input[name="js_osc_delivery_type"]').click(function (ev) {
    ...
    $payment.on('click', 'input[name="acquirer"]',function(ev) {
      ...
      $('.oe_sale_acquirer_button[data-id]', '.o_osc_order').addClass('hidden');
      $('.oe_sale_acquirer_button[data-id="' + payment_id + '"]', '.o_osc_order').removeClass('hidden');
    ...
    // ajax call
    $('.o_osc_order .js_payment').on('click', 'form button[type=submit]', function(ev) {
      ev.preventDefault();
      ev.stopPropagation();
      validateAddress(ev);
      return false;
