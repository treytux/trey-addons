...
class TestContractLineLot(common.TransactionCase):
    ...
    def test_contract_line_without_date_end(self):
        ...
        self.create_inventory(
            self.product_01, location, 10, lot_id=self.lot_01.id)
        ...
        self.assertEqual(self.lot_01.last_location_id, location)

    def test_contract_multiple_lines_with_lot(self):
        location = self.env.ref('stock.stock_location_stock')
        self.create_inventory(
            self.product_01, location, 1, lot_id=self.lot_01.id)
        self.create_inventory(
            self.product_02, location, 1, lot_id=self.lot_02.id)
        contract = self.env['contract.contract'].create({
            'name': 'Test contract',
            'partner_id': self.partner.id,
        })
        ...
        self.assertEqual(self.lot_02.last_location_id, location)
