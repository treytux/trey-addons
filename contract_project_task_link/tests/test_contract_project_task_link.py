...
class TestSaleOrderPurchaseOrderLink(common.TransactionCase):
    ...
    def test_contract_and_project_from_sale(self):
        ...
        self.assertEquals(sale.tasks_count, 0)
        self.assertEquals(len(self.project.task_ids), 0)
        sale.action_confirm()
        self.assertEquals(sale.tasks_count, 1)
        self.assertEquals(len(self.project.task_ids), 1)
        contract = self.env['contract.contract'].search([
            ('partner_id', '=', self.partner.id),
        ])
        self.assertEquals(len(contract), 1)
        self.assertEquals(contract.task_count, 1)
        self.assertEquals(
            contract.contract_line_ids.mapped('sale_order_line_id'),
            sale.order_line)
        self.assertEquals(self.project.task_ids.sale_line_id, sale.order_line)
    ...
    def test_create_only_project_from_sale(self):
        ...
        self.assertEquals(len(contract), 0)
        self.assertEquals(contract.task_count, 0)
    ...
    def test_create_only_contract_from_sale(self):
        ...
        self.assertEquals(
            contract.contract_line_ids.mapped('sale_order_line_id'),
            sale.order_line)
        self.assertNotEquals(self.project.task_ids.sale_line_id, sale.order_line)
