...
class TestSaleInvoicePolicyAllDelivered(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.dropshipping_type = self.env.ref(
    ...
    def test_sale_no_shipment_02(self):
        ...
        self.assertEqual(len(sale.picking_ids), 2)
        self.assertEqual(sale.picking_ids.mapped('state'), ['cancel', 'done'])
        self.assertEqual(
            sale.picking_ids[0].move_lines[0].state, 'cancel')
        self.assertEqual(
            sale.picking_ids[0].move_lines[0].product_uom_qty, 1)
        self.assertEqual(
            sale.picking_ids[1].move_lines[0].state, 'done')
        self.assertEqual(
            sale.picking_ids[1].move_lines[0].product_uom_qty, 1)
    ...
    def test_sale_one_order_line_no_shipment_01(self):
        ...
        self.assertEqual(sale.picking_ids.mapped('state'), ['assigned', 'done'])
        self.assertEqual(
            sale.picking_ids[0].move_lines[0].state, 'partially_available')
        self.assertEqual(
            sale.picking_ids[0].move_lines[0].product_uom_qty, 9)
        self.assertEqual(
            sale.picking_ids[1].move_lines[0].state, 'done')
        self.assertEqual(
            sale.picking_ids[1].move_lines[0].product_uom_qty, 1)
    ...
    def test_sale_dropshipping_partial_cancel(self):
        ...
        self.assertEqual(sale.picking_ids[0], purchase.picking_ids[0])
        purchase_picking = purchase.picking_ids[0]
        purchase_picking.move_lines[0].quantity_done = 1
        wizard = self.env['stock.backorder.confirmation'].create({
    ...
    def test_sale_dropshipping_partial_return_partial(self):
        ...
        self.assertEqual(sale.picking_ids[0], purchase.picking_ids[0])
        purchase_picking = purchase.picking_ids[0]
        purchase_picking.move_lines[0].quantity_done = 2
        wizard = self.env['stock.backorder.confirmation'].create({
            'pick_ids': [(4, purchase_picking.id)],
        })
        wizard.process()
