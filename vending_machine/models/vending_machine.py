...
class VendingMachine(models.Model):
    ...
    def _get_stock_data_from_api(self):
        try:
            ...
            response = requests.post(api_url, data=soap_request,
                                     headers=headers)
            ...
            stock_data_list = []
            stock_data_elements = root.findall('.//ns:CpxType_StockData',
                                               namespace)
            for stock_data in stock_data_elements:
                try:
                    product_code = stock_data.find('ns:ProductCode',
                                                   namespace).text
                    current_stock = stock_data.find('ns:CurrentStock',
                                                    namespace).text or 0
                    desired_stock = stock_data.find('ns:DesiredStock',
                                                    namespace).text or 0
                    selection_number = stock_data.find('ns:SelectionNumber',
                                                       namespace).text
    ...
    def _get_moves_dispensing_moves(self, from_batch, to_batch):
        try:
            ...
            <soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
                        ...
                        xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
                <soap:Body>
                    <GetExtendedDispensationData xmlns="http://compendia.jofemar.com/">
                    <CompanyId>{self.company_id}</CompanyId>
            ...
            product_codes_from_api = {
                el.find('ns:ProductCode', namespace).text
                for el in move_elements
                if el.find('ns:ProductCode', namespace) is not None
            }
            products_odoo = self.env['product.product'].search([
                ('default_code', 'in', list(product_codes_from_api)),
            ])
            products_map = {
                product.default_code: product for product in products_odoo}
            moves = []
    ...
    def get_dispensing_moves(self, date_from, date_to, unified_invoice=False):
        ...
        while True:
            moves = self._get_moves_dispensing_moves(batch, batch)
            if moves is False:
                break
            force_break = False
            for move in moves:
                ...
                if move['DispenseType'] in ['20', '21', '22', '26', '31']:
                    continue
                all_moves.setdefault(move['product_id'], []).append(move)
            ...
            if force_break:
                break
            batch += 1
        self.initial_batch = initial_batch or batch
        product_sum = {k: len(v) for k, v in all_moves.items()}
        return product_sum, machine_ids
    ...
    def action_move_to_deposit(self, date_from, date_to):
        ...
        moves_machine, machine_codes = self.get_dispensing_moves(
            date_from=date_from,
            date_to=date_to,
            unified_invoice=unified_invoice)
        time.sleep(15)
        moves_machine2, machine_codes2 = self.get_dispensing_moves(
            date_from=date_from,
            date_to=date_to,
            unified_invoice=unified_invoice)
        ...
        action = self.env.ref('stock_deposit.stock_deposit_action').read()[0]
    ...
    def update_machine_stock(self, stock_data_array):
        ...
        try:
            ...
            for product_data in stock_data_array:
                ...
                ET.SubElement(stock_data_item,
                              '{' + jofemar_ns + '}SelectionNumber'
                              ).text = str(product_data.get('SelectionNumber'))
                ET.SubElement(stock_data_item,
                              '{' + jofemar_ns + '}ProductCode'
                              ).text = product_data.get('ProductCode')
                ET.SubElement(stock_data_item,
                              '{' + jofemar_ns + '}CurrentStock'
                              ).text = str(product_data.get('CurrentStock'))
                ET.SubElement(stock_data_item,
                              '{' + jofemar_ns + '}CurrentStockAsRelativeValue'
                              ).text = 'true'
                ET.SubElement(stock_data_item,
                              '{' + jofemar_ns + '}DesiredStock'
                              ).text = str(product_data.get('DesiredStock'))
            ET.SubElement(set_stock_data, '{' + jofemar_ns + '}RequestDateTime'
                          ).text = request_datetime
            ET.SubElement(set_stock_data, '{' + jofemar_ns + '}Signature'
                          ).text = signature_hash
            soap_request = ET.tostring(envelope,
