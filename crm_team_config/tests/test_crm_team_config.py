...
class TestCrmTeamConfig(TransactionCase):

    def setUp(self):
        super(TestCrmTeamConfig, self).setUp()
        ...
        def _mock_check_multiwarehouse_group(*args, **kwargs):
            pass

        with patch(
            'odoo.addons.stock.models.stock_warehouse.'
            'Warehouse._check_multiwarehouse_group',
            new=_mock_check_multiwarehouse_group,
        ):
            ...
            self.inventory_wh1 = self.create_inventory(self.warehouse1)
            self.inventory_wh2 = self.create_inventory(self.warehouse2)
    ...
    def test_check_access_warehouse_03(self):
        ...
        self.assertIn(
            self.warehouse1, self.user_warehouse.sale_team_id.warehouse_ids)
        with self.assertRaises(AccessError):
            self.warehouse2.with_user(
                self.user_warehouse).browse(self.warehouse2.id).name
        self.warehouse2.with_user(self.manager_warehouse).browse(
            self.warehouse2.id).name

    def test_check_access_picking_type_01(self):
        self.warehouse1.out_type_id.with_user(self.manager_warehouse).browse(
            self.warehouse1.out_type_id.id).name
        with self.assertRaises(AccessError):
            self.warehouse1.out_type_id.with_user(self.user_warehouse).browse(
                self.warehouse1.out_type_id.id).name
    ...
    def test_check_access_picking_type_03(self):
        ...
        self.assertIn(
            self.warehouse1, self.user_warehouse.sale_team_id.warehouse_ids)
        with self.assertRaises(AccessError):
            self.warehouse2.out_type_id.with_user(self.user_warehouse).browse(
                self.warehouse2.out_type_id.id).name
        self.warehouse2.out_type_id.with_user(self.manager_warehouse).browse(
            self.warehouse2.out_type_id.id).name

    def test_check_access_picking_01(self):
        self.picking1.with_user(
    ...
    def test_check_access_picking_02(self):
        ...
        self.picking1.with_user(
            self.user_warehouse).browse(self.picking1.id).name
        self.picking1.with_user(self.manager_warehouse).browse(
    ...
    def test_check_access_orderpoint_02(self):
        ...
        self.assertIn(
            self.warehouse1, self.user_warehouse.sale_team_id.warehouse_ids)
        self.orderpoint_wh1.with_user(self.user_warehouse).browse(
            self.orderpoint_wh1.id).name
        self.orderpoint_wh1.with_user(self.manager_warehouse).browse(
            self.orderpoint_wh1.id).name
    ...
    def test_check_access_orderpoint_03(self):
        ...
        self.assertIn(
            self.warehouse1, self.user_warehouse.sale_team_id.warehouse_ids)
        with self.assertRaises(AccessError):
            self.orderpoint_wh2.with_user(
                self.user_warehouse).browse(self.orderpoint_wh2.id).name
        self.orderpoint_wh2.with_user(self.manager_warehouse).browse(
    ...
    def test_check_access_location_01(self):
        ...
        location_wh1.with_user(
            self.manager_warehouse).browse(location_wh1.id).name
        with self.assertRaises(AccessError):
            location_wh1.with_user(
                self.user_warehouse).browse(location_wh1.id).name
    ...
    def test_check_access_location_02(self):
        ...
        self.assertIn(
            self.warehouse1, self.user_warehouse.sale_team_id.warehouse_ids)
        location_wh1 = self.warehouse1.out_type_id.default_location_src_id
        location_wh1.with_user(self.user_warehouse).browse(
            location_wh1.id).name
        location_wh1.with_user(self.manager_warehouse).browse(
            location_wh1.id).name
    ...
    def test_check_access_location_03(self):
        ...
        with self.assertRaises(AccessError):
            location_wh2.with_user(
                self.user_warehouse).browse(location_wh2.id).name
        location_wh2.with_user(
            self.manager_warehouse).browse(location_wh2.id).name

    def test_check_access_inventory_01(self):
        self.inventory_wh1.with_user(self.manager_warehouse).browse(
            self.inventory_wh1.id).display_name
        with self.assertRaises(AccessError):
            self.inventory_wh1.with_user(self.user_warehouse).browse(
                self.inventory_wh1.id).display_name
    ...
    def test_check_access_inventory_03(self):
        ...
        self.assertIn(
            self.warehouse1, self.user_warehouse.sale_team_id.warehouse_ids)
        with self.assertRaises(AccessError):
            self.inventory_wh2.with_user(self.user_warehouse).browse(
                self.inventory_wh2.id).display_name
        self.inventory_wh2.with_user(self.manager_warehouse).browse(
            self.inventory_wh2.id).display_name

    def test_check_access_account_journal_01(self):
        self.check_journal.with_user(
    ...
    def test_check_access_invoice_journal_03(self):
        ...
        self.assertIn(
            ...
            self.user_warehouse.sale_team_id.invoice_journal_ids)
        with self.assertRaises(AccessError):
            self.sale_journal2.with_user(
                self.user_warehouse).browse(self.sale_journal2.id).name
        self.sale_journal2.with_user(self.manager_warehouse).browse(
    ...
    def test_check_update_locations_when_create_location_01(self):
        ...
        location_test = self.env['stock.location'].with_user(
    ...
    def test_check_update_locations_when_write_location_01(self):
        ...
        self.assertTrue(location_def_wh1.replenish_location)
        location_def_wh1.replenish_location = False
        self.assertFalse(location_def_wh1.replenish_location)
