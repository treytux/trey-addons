...
class EduWizardFillSubjects(models.TransientModel):
    ...
    def button_accept(self):
        ...
        if len(enrollments) > 1:
            raise exceptions.ValidationError(_(
                'There are duplicate active enrollments for this '
                'student: %s') % student.name)
        if not enrollments:
            raise exceptions.ValidationError(_(
                'There are not active enrollments for this student: %s'
            ) % (student.name))
        bulletin_domain = [
            ('name', '=', student.id),
        ...
        if not bulletin:
            raise exceptions.ValidationError(_(
                'There are not bulletins for this student: %s'
            ) % (student.name))
        if not training_plan.line_ids:
            raise exceptions.ValidationError(_(
                'There are not subjects configured in trainign plan: %s'
            ) % training_plan.name)
        plan_lines = training_plan.line_ids.filtered(
            lambda line: line.subject_id in
            enrollments.enrollment_line_ids.subject_id)
        plan_lines.create_evaluation_lines(
