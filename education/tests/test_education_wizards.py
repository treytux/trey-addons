...
class TestEducationWizard(TestBase):
    def setUp(self):
        super().setUp()
        data = {
            'date': '2017-05-21',
    ...
    def test_education_wizard_fill_subjects(self):
        ...
        self.assertEqual(len(bulletin.evaluation_line_ids), 2)
        self.assertEqual(
    ...
    def test_education_wizard_enroll_student(self):
        ...
        self.assertEqual(len(wizard.student_enrolled_ids), 0)
        self.assertEqual(len(wizard.student_ids), 1)
        self.assertEqual(wizard.student_ids, self.student_01)
        wizard.button_enroll()
        self.assertEqual(enrollment.state, 'active')
        self.assertEqual(enrollment.classroom_id.id, self.classroom_01.id)

    def test_education_wizard_generate_bulletins(self):
        self.enrollment_01.to_active()
        self.enrollment_01.fill_subjects()
        training_plan_line = self.training_plan_01.line_ids
        wizard_ctx = {
            'active_model': 'edu.training.plan.line',
            'active_ids': training_plan_line.ids,
        }
