...
class ProductProduct(models.Model):
    ...
    def _woo_convert_dict_images(self, website):
        if not self.image:
            return []
        image_tmpl = '%s/website/image/product.product/%s/image/product.jpg'
        images = [
            {'src': image_tmpl % (website.woo_odoo_url, self.id)},
        ]
        image_tmpl = (
            '%s/website/image/product.image/%s/image/product.jpg'
        )
        for img in self.product_image_ids:
            images.append(
                {'src': image_tmpl % (website.woo_odoo_url, img.id)},
            )
    ...
    def woo_upload_dict_get(self, update_fields=None):
        ...
        if not update_fields:
            update_fields = ['image', 'product_image_ids']
        ...
        if not self.is_published:
            ...
            if 'website_published' in update_fields:
                data['status'] = 'publish'
            images_fields = ['image', 'product_image_ids']
        ...
        self = self.with_context(pricelist=pricelist.id)
        ...
        data.update({
            ...
            'regular_price': str(float_round(self.price, precision)),
    ...
    def woo_sync_export_records(self, website, products=None):
        ...
        if website.woo_catalog_ids:
            domain += (('catalog_ids', 'in', website.woo_catalog_ids.ids),)
        else:
            domain += [
                '|',
    ...
    def woo_sync_import_record(self, website, data):
        ...
        return self.create({
            ...
            'image': images and images_get(images[0][0]) or False,
            'accessory_product_ids': [(6, 0, cross_ids)],
            'public_categ_ids': [(6, 0, categs)],
            'tag_ids': [(6, 0, tags)],
            'product_image_ids': [
                (0, 0, {'image': images_get(im[0]), 'name': im[1]})
                for im in images[1:]],
        })
    ...
    def action_woo_upload(self):
        ...
        for website in websites:
            products = self.filtered(
                lambda p: not p.website or website == p.website_id)
