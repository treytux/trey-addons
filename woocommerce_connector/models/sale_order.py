...
class SaleOrder(models.Model):
    ...
    def woo_partner_browse_or_create(self, website, data, meta_data=None,
                                     parent=None, partner_type=False,
        ...
        if partner:
            if data.get('email') and data['email'] not in partner.email:
    ...
    def woo_sync_import_record(self, website, data, refunds=None):
        ...
        sale_data = {
            ...
            'website_id': website.id,
            'company_id': website.company_id.id,
            'date_order': self.woo_format_str_to_date(data['date_created_gmt']),
        ...
        for item in data['fee_lines']:
            ...
            if not product:
                errors.append(
                    _('Product for fee cost <b>%s</b> not exist, please '
        ...
        payment = self.env['payment.acquirer'].search([
        ...
        if not payment:
            ...
            self.env['payment.transaction'].create({
                ...
                'partner_country_id': (
                    ...
                    if self.partner_id.country_id.id
                    else self.env.user.company_id.country_id.id
                ),
            })
