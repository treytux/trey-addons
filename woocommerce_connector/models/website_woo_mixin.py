...
class WebsiteWooMixin(models.AbstractModel):
    ...
    def woo_rpc_call(self, method, endpoint, data=None, website=None):
        def message(txt):
            return '(%s) %s | %s' % (endpoint, txt, data)
        ...
        try:
            ...
            raise WooError(
                message('Not return JSON %s' % res.content), res.content)
        if 200 <= res.status_code >= 299:
            msg = message(_('%s error in the %s operation to %s: %s') % (
                self._name, method, endpoint, res_data['message']))
            raise WooError(msg, res_data)
        if isinstance(res_data, dict) and res_data.get('code'):
            msg = message('[%s] %s' % (res_data['code'], res_data['message']))
            raise WooError(msg, res_data)
    ...
    def woo_post_error(self, message):
        _log.error(message)
        message += _(
            '<br/>This error is referenced with %s'
            '<a href=# data-oe-model=%s data-oe-id=%d>%s</a>') % (
                self._description, self._name, self.id, self['name'])
        website = self._website_get()
        website.salesteam_id.message_post(
            body=message,
            subject='Woocommere %s connector error' % website.name
        )
    ...
    def create(self, vals):
        ...
        for website in websites:
            if not self.woo_sync_export_records(website, products=self):
                response.woo_upload(website)
        return response
    ...
    def woo_sync_export(self, website, products=None):
        ...
        for index, record in enumerate(records['upload']):
            _log.info('[%s/%s] %s "%s" upload to website "%s"' % (
                index + 1, len(records['upload']), record._name, record.name,
                website.name))
            record.woo_upload(website)
        for index, record in enumerate(records['delete']):
            _log.info('[%s/%s] %s "%s" delete to website "%s"' % (
                index + 1, len(records['delete']), record._name, record.name,
                website.name))
