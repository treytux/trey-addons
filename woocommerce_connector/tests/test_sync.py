...
class TestSync(common.TestCommon):
    ...
    def test_woo_sync_import(self):
        ...
        self.assertEquals(wizard.website_id, self.website)
        wizard.action_count()
        self.assertEquals(wizard.import_product_count, 0)
    ...
    def test_woo_product_sync(self):
        ...
        self.product.website_publish = False
        products = self.product.woo_sync_export_records(self.website)
        self.assertIn(self.product, products)
        self.product.website_publish = True
    ...
    def test_woocommerce_upload_wizard(self):
        ...
        self.assertEquals(wizard.product_count, 1)
        self.assertEquals(wizard.public_category_count, 2)
        self.assertEquals(wizard.product_tag_count, 2)
        self.assertEquals(wizard.del_product_count, 0)
        self.assertEquals(wizard.del_public_category_count, 0)
        self.assertEquals(wizard.del_product_tag_count, 0)
        info_categs = self.env['product.public.category']._woo_sync_export_get(
            self.product)
        self.assertEquals(len(info_categs['delete']), 0)
        self.assertEquals(self.product.woo_get_id(), None)
        wizard.action_sync()
