...
class TestMixin(common.TestCommon):
    ...
    def test_check_url(self):
        def check_connection(website, path=''):
            url = '%s/wp-json/%s/%s' % (
                self.website.woo_url, self.website.woo_version, path)
    ...
    def test_woo_product_categ(self):
        ...
        self.assertEqual(self.categ.woo_get_id(), None)
        self.assertEqual(self.categ_child.woo_get_id(), None)
    ...
    def test_woo_product_price(self):
        ...
        data = self.product.with_context(
            website=self.website, pricelist=pricelist.id
        ).woo_upload_dict_get()
