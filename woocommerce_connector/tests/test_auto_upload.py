...
class TestAutoUpload(common.TestCommon):
    ...
    def test_auto_upload_orm(self):
        ...
        self.assertEquals(weights[products[0].default_code], '99.0')
        self.assertEquals(weights[products[1].default_code], '99.0')
        products.unlink()
        woo_products = self.product.woo_rpc_get()
    ...
    def test_auto_upload_queue(self):
        ...
        jobs = job_obj.search([('method_name', '=', 'woo_rpc_upload_job')])
        self.assertEqual(len(jobs), 1)
        self.product.woo_set_id(999)
        self.product.unlink()
        jobs = job_obj.search([('method_name', '=', 'woo_rpc_delete_job')])
        self.assertEqual(len(jobs), 1)
        product_obj = self.env['product.product'].with_context(
            website=self.website)
        product_obj.create({
