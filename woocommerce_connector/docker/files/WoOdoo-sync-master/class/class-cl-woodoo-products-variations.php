 ...
 */
class CL_WooDoo_Products_Variations extends WC_REST_Product_Variations_Controller {
	...
	public function create_item( $request ) {
		$id_odoo_parent = $request->get_param( 'id_odoo_parent' );
		$id_odoo        = $request->get_param( 'id_odoo' );
		$lang           = $request->get_param( 'lang' );
		$attributes     = $request->get_param( 'attributes' );

		// Check if id_odoo and lang are set.
		if ( ! $id_odoo_parent || ! $lang || ! $attributes[0]['slug'] ) {
			$error = new WP_Error( 'required_fields', 'Must send lang and id_odoo and attributes.' );
		...
		$check_product = check_if_exists_product_odoo( $id_odoo_parent );
		if ( null !== $check_product ) {
			...
			if ( ! $id_wp ) {
				...
				$error = new WP_Error( 'product_not_exists', 'Product not exists, create first' );
				wp_send_json_error( $error );
			}
			$request->set_param( 'product_id', $id_wp );
		}
		...
		$check_product = check_if_exists_variation_odoo( $id_odoo );
		if ( null !== $check_product ) {
			// Check if element exists or translation.
			$id_wp = apply_filters( 'wpml_object_id', $check_product['post_id'], 'product_variation', false, $lang );
	...
	private function add_bulk_prices( $id_wp, $prices ) {
		...
		foreach ( $prices[0] as $role => $rules ) {
			$post = array(
