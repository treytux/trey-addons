 ...
 */
class CL_WooDoo_Categories extends WC_REST_Product_Categories_Controller {
	...
	public function create_item( $request ) {
		...
		$check_cat = check_if_exists_category_odoo( $id_odoo );

		if ( null !== $check_cat ) {
			// Check if element exists or translation.
			$id_wp = apply_filters( 'wpml_object_id', $check_cat['term_id'], 'product_cat', false, $lang );
			if ( $id_wp ) {
		...
		if ( $request->get_param( 'parent' ) ) {
			$parent = get_category_id_by_idodoo( $request->get_param( 'parent' ), $lang );
			$request->set_param( 'parent', $parent['term_id'] );
		}

		add_action( 'woocommerce_rest_insert_product_cat', array( $this, 'update_term_meta_odoo' ) );
		$created_tax = parent::create_item( $request );
		remove_action( 'woocommerce_rest_insert_product_cat', array( $this, 'update_term_meta_odoo' ) );
		...
		$wpml_element_type = apply_filters( 'wpml_element_type', 'tax_product_cat' );
