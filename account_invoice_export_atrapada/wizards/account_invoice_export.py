...
class AccountInvoiceExport(models.TransientModel):
    ...
    def get_invoice_lines(self, invoice, table_dict, current_row):
        table_dict.update({
            current_row: [
                _('Reference'), _('Intrastat'), _('Description'),
                _('Quantity'), _('Price unit'), _('Discount'),
                _('Price subtotal')]})
        current_row += 1
        row_by_tmpl = {}
        for line in invoice.invoice_line:
            ...
            intrastat = (
                line.product_id and line.product_id.hs_code_id and
                line.product_id.hs_code_id.hs_code or '')
            row_vals = [
                ref, intrastat, description, line.quantity, line.price_unit,
            ...
            if product_tmpl_id not in row_by_tmpl:
                ...
                price_diff = (
                    line.price_unit != row_by_tmpl[product_tmpl_id][4])
                disc_diff = line.discount != row_by_tmpl[product_tmpl_id][5]
                if price_diff or disc_diff:
                    diff = line.price_unit if price_diff else line.discount
                    composed_key = '%s-%s' % (product_tmpl_id, diff)
                    if composed_key in row_by_tmpl:
                        row_by_tmpl[composed_key][3] += line.quantity
                        row_by_tmpl[composed_key][6] += line.price_subtotal
                        continue
                    row_by_tmpl[composed_key] = row_vals
                    table_dict[current_row] = row_by_tmpl[composed_key]
                    current_row += 1
                else:
                    row_by_tmpl[product_tmpl_id][3] += line.quantity
                    row_by_tmpl[product_tmpl_id][6] += line.price_subtotal
        current_row += 1
