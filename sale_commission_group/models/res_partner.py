...
class ResPartner(models.Model):
    ...
    @api.depends('agent_ids', 'agent_ids.name')
    def _compute_agents_name(self):
        for partner in self:
            agent_names = {
                agent.name for agent in partner.agent_ids if agent.name
            }
            partner.agents_name = ', '.join(agent_names)
    ...
    def _compute_agent_customers(self):
        for agent in self:
            agent_customers = self.env['res.partner'].search(
                agent._get_customers_domain(), order='name ASC')
