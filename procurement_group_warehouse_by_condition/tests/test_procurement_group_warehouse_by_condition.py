...
class TestProcurementGroupWarehouseByCondition(TransactionCase):
    ...
    def setUp(self):
        ...
        self.customer = self.env['res.partner'].with_user(
            self.user.id).create({
                'name': 'Customer test',
                'company_id': False,
                'zip': '18008',
            })
        self.supplier = self.env['res.partner'].with_user(
            self.user.id).create({
                'name': 'Supplier test',
                'company_id': False,
            })
        ...
        self.product_01 = self.env['product.product'].with_user(
            self.user.id).create({
                'type': 'product',
                'company_id': False,
        ...
        self.env['product.supplierinfo'].with_user(
            self.user.id).create({
                'partner_id': self.supplier.id,
                'product_tmpl_id': self.product_01.product_tmpl_id.id,
                'price': 80,
            })
        self.product_02 = self.env['product.product'].with_user(
            self.user.id).create({
                'type': 'product',
                'company_id': False,
        ...
        self.product_03 = self.env['product.product'].with_user(
            self.user.id).create({
                'type': 'product',
                'company_id': False,
    ...
    def create_sale(self, product, quantity, warehouse, partner, user):
        return self.env['sale.order'].with_user(user.id).create({
    ...
    def create_warehouse(self, key, user):
        return self.env['stock.warehouse'].with_user(user.id).create({
    ...
    def test_normal_flow_without_warehouse_by_condition(self):
        ...
        self.assertEqual(picking.move_ids.product_id, self.product_01)
        self.assertEqual(
            picking.move_ids.location_id, self.stock_wh.lot_stock_id)
        self.assertEqual(
            picking.move_ids.location_dest_id, self.customer_loc)
    ...
    def test_cond_with_stock_in_main_warehouse_variant(self):
        self.update_qty_on_hand(
            self.product_01, self.warehouse_02.lot_stock_id, 10)
        self.env['procurement.group.warehouse_by_condition'].with_user(
        ...
        self.assertEqual(picking.move_ids.product_id, self.product_01)
        self.assertEqual(
            picking.move_ids.location_id, self.warehouse_02.lot_stock_id)
        self.assertEqual(
            picking.move_ids.location_dest_id, self.customer_loc)
        self.assertEqual(
    ...
    def test_cond_without_stock_in_main_warehouse_variant(
            self):
    ...
    def test_cond_and_several_zip_variant(self):
        self.update_qty_on_hand(
            self.product_01, self.warehouse_02.lot_stock_id, 10)
        self.env['procurement.group.warehouse_by_condition'].with_user(
            self.user_manager).create({
    ...
    def test_cond_and_zip_variant(self):
        self.update_qty_on_hand(
        ...
        self.assertEqual(picking.move_ids.product_id, self.product_01)
        self.assertEqual(
            picking.move_ids.location_id, self.warehouse_03.lot_stock_id)
        self.assertEqual(
            picking.move_ids.location_dest_id, self.customer_loc)
        self.assertEqual(
    ...
    def test_cond_and_alternative_warehouse_with_stock_variant(self):
    ...
    def test_cond_and_alternative_warehouse_without_stock_variant(
            self):
        self.main_company.notification_user_id = self.user_manager.id
        self.assertEqual(self.product_01.with_context(
            location=self.stock_wh.lot_stock_id.id).qty_available, 0)
    ...
    def test_constraint_warehouse_id_unique_variant(self):
        ...
        self.assertIn(
            ...
            'warehouse Warehouse by conditions.', result.exception.args[0])
    ...
    def test_cond_and_zip_and_alternative_whs_with_stock_variant(
            self):
        self.assertEqual(self.product_01.with_context(
            location=self.stock_wh.lot_stock_id.id).qty_available, 0)
        self.update_qty_on_hand(
    ...
    def test_cond_and_zip_and_alternative_whs_without_stock_variant(
            self):
        self.main_company.notification_user_id = self.user_manager.id
        self.assertEqual(self.product_01.with_context(
            location=self.stock_wh.lot_stock_id.id).qty_available, 0)
    ...
    def test_cond_several_sale_lines_variant(self):
        self.update_qty_on_hand(
            self.product_01, self.warehouse_02.lot_stock_id, 10)
        self.update_qty_on_hand(
        ...
        sale = self.env['sale.order'].with_user(self.user.id).create({
        ...
        self.assertEqual(
            picking_product_01.move_ids.location_id,
            self.warehouse_02.lot_stock_id)
        self.assertEqual(
            picking_product_01.move_ids.location_dest_id, self.customer_loc)
        self.assertEqual(
        ...
        self.assertEqual(
            picking_product_02.move_ids.location_id,
            self.warehouse_03.lot_stock_id)
        self.assertEqual(
            picking_product_02.move_ids.location_dest_id, self.customer_loc)
        self.assertEqual(
        ...
        self.assertEqual(len(mail_messages), 2)

    def test_cond_partial_stock_warehouses_01_variant(self):
        self.update_qty_on_hand(
        ...
        self.assertEqual(len(picking_stock_wh.move_ids), 1)
        self.assertEqual(picking_stock_wh.move_ids.product_uom_qty, 1)
        ...
        self.assertEqual(len(picking_stock_wh_02.move_ids), 1)
        self.assertEqual(picking_stock_wh_02.move_ids.product_uom_qty, 2)
    ...
    def test_cond_partial_stock_warehouses_02_variant(self):
        self.update_qty_on_hand(
            self.product_01, self.stock_wh.lot_stock_id, 1)
        self.update_qty_on_hand(
        ...
        self.assertEqual(len(picking_stock_wh_02), 1)
        self.assertEqual(len(picking_stock_wh_02.move_ids), 1)
        self.assertEqual(picking_stock_wh_02.move_ids.product_uom_qty, 1)
        ...
        self.assertEqual(len(picking_stock_wh_03.move_ids), 1)
        self.assertEqual(picking_stock_wh_03.move_ids.product_uom_qty, 1)
        picking_stock_wh.action_assign()
        self.assertEqual(picking_stock_wh.state, 'assigned')
        picking_stock_wh_02.action_assign()
        self.assertEqual(picking_stock_wh_02.state, 'assigned')
    ...
    def test_cond_partial_stock_warehouses_03_variant(self):
        self.assertEqual(self.product_01.with_context(
            location=self.stock_wh.lot_stock_id.id).qty_available, 0)
        self.update_qty_on_hand(
            self.product_01, self.warehouse_02.lot_stock_id, 1)
    ...
    def test_cond_no_satisfy_any_condition_variant(self):
        self.main_company.notification_user_id = self.user_manager.id
        self.env['procurement.group.warehouse_by_condition'].with_user(
            self.user_manager).create({
                'warehouse_id': self.warehouse_by_condition.id,
        ...
        self.assertEqual(mail_activities.user_id, self.user_manager)
        mail_messages = self.env['mail.message'].search([
            ('model', '=', 'sale.order'),
            ('res_id', '=', sale.id),
            ('body', 'ilike', (
    ...
    def test_cond_no_satisfy_any_condition_zip_variant(self):
        self.main_company.notification_user_id = self.user_manager.id
        self.env['procurement.group.warehouse_by_condition'].with_user(
            self.user_manager).create({
                'warehouse_id': self.warehouse_by_condition.id,
    ...
    def test_multicompany_notification_user(self):
        ...
        self.assertEqual(
            self.product_01.with_user(self.user_manager).with_context(
        ...
        self.assertEqual(picking.move_ids.product_id, self.product_01)
        self.assertEqual(
            picking.move_ids.location_id, warehouse_company_2.lot_stock_id)
        self.assertEqual(
            picking.move_ids.location_dest_id, self.customer_loc)
        self.assertEqual(
    ...
    def test_apply_condition_with_mto_route(self):
        ...
        self.assertEqual(len(picking_out.move_ids), 1)
        self.assertEqual(picking_out.move_ids.product_id, self.product_01)
        self.assertEqual(
            picking_out.move_ids.location_id, self.warehouse_02.lot_stock_id)
        self.assertEqual(
            picking_out.move_ids.location_dest_id, self.customer_loc)
        self.assertEqual(
        ...
        self.assertEqual(len(picking_in.move_ids), 1)
        self.assertEqual(picking_in.move_ids.product_id, self.product_01)
        self.assertEqual(picking_in.move_ids.location_id, self.supplier_loc)
        self.assertEqual(
            picking_in.move_ids.location_dest_id,
            self.warehouse_02.lot_stock_id)
        self.assertEqual(
    ...
    def test_not_apply_condition_dropshipping_route(self):
        ...
        self.assertEqual(len(picking.move_ids), 1)
        self.assertEqual(picking.move_ids.product_id, product_dropshipping)
        self.assertEqual(picking.move_ids.location_id, self.supplier_loc)
        self.assertEqual(
            picking.move_ids.location_dest_id, self.customer_loc)
    ...
    def test_condition_equal(self):
        ...
        self.assertEqual(
            picking.move_ids.location_id, self.stock_wh.lot_stock_id)
        self.assertEqual(picking.move_ids.location_dest_id, self.customer_loc)
        self.assertEqual(picking.picking_type_id, self.stock_wh.out_type_id)
        picking.action_assign()
        self.assertEqual(picking.state, 'confirmed')
        mail_messages = self.env['mail.message'].search([
    ...
    def test_cond_with_stock_in_main_warehouse_template(self):
        self.update_qty_on_hand(
            self.product_01, self.warehouse_02.lot_stock_id, 10)
        self.env['procurement.group.warehouse_by_condition'].with_user(
            self.user_manager).create({
        ...
        self.assertEqual(picking.move_line_ids.product_id, self.product_01)
        self.assertEqual(
            picking.move_line_ids.location_id, self.warehouse_02.lot_stock_id)
        self.assertEqual(
            picking.move_line_ids.location_dest_id, self.customer_loc)
        self.assertEqual(
            picking.picking_type_id, self.warehouse_02.out_type_id)
        picking.action_assign()
        self.assertEqual(picking.state, 'assigned')
    ...
    def test_cond_with_stock_in_main_warehouse_diff_applied_on_01(
            self):
        self.update_qty_on_hand(
            self.product_01, self.warehouse_02.lot_stock_id, 10)
        self.env['procurement.group.warehouse_by_condition'].with_user(
    ...
    def test_cond_with_stock_in_main_warehouse_diff_applied_on_02(
            self):
        self.update_qty_on_hand(
            self.product_01, self.warehouse_03.lot_stock_id, 10)
        self.env['procurement.group.warehouse_by_condition'].with_user(
        ...
        self.assertEqual(picking.move_line_ids.product_id, self.product_01)
        self.assertEqual(
            picking.move_line_ids.location_id, self.warehouse_03.lot_stock_id)
        self.assertEqual(
            picking.move_line_ids.location_dest_id, self.customer_loc)
        self.assertEqual(
            picking.picking_type_id, self.warehouse_03.out_type_id)
    ...
    def test_cond_with_stock_in_main_warehouse_diff_applied_on_03(
            self):
        self.update_qty_on_hand(
            self.product_01, self.warehouse_02.lot_stock_id, 10)
        self.env['procurement.group.warehouse_by_condition'].with_user(
    ...
    def test_cond_with_stock_in_main_warehouse_diff_applied_on_04(
            self):
        self.update_qty_on_hand(
            self.product_01, self.warehouse_03.lot_stock_id, 10)
        self.env['procurement.group.warehouse_by_condition'].with_user(
    ...
    def test_cond_with_stock_in_main_warehouse_diff_applied_on_05(
            self):
        self.update_qty_on_hand(
            self.product_01, self.warehouse_02.lot_stock_id, 10)
        self.env['procurement.group.warehouse_by_condition'].with_user(
    ...
    def test_cond_alternative_warehouse_virtual_qtyfield_with_stock_variant(
            self):
        self.main_company.product_qty_field = 'virtual_available'
        self.assertEqual(self.product_01.with_context(
            location=self.stock_wh.lot_stock_id.id).qty_available, 0)
        self.assertEqual(self.product_01.with_context(
            location=self.warehouse_02.lot_stock_id.id).qty_available, 0)
        self.assertEqual(self.product_01.with_context(
            location=self.stock_wh.lot_stock_id.id).virtual_available, 0)
        self.assertEqual(self.product_01.with_context(
            location=self.warehouse_02.lot_stock_id.id).virtual_available, 0)
        self.update_qty_on_hand(
            self.product_01, self.warehouse_03.lot_stock_id, 10)
        self.env['procurement.group.warehouse_by_condition'].with_user(
            self.user_manager).create({
        ...
        self.assertEqual(len(mail_messages), 1)
        self.assertEqual(self.product_01.with_context(
            location=self.warehouse_03.lot_stock_id.id).qty_available, 10)
        self.assertEqual(self.product_01.with_context(
            location=self.warehouse_03.lot_stock_id.id).virtual_available, 9)
        sale2 = self.create_sale(
            self.product_01, 10, self.warehouse_by_condition, self.customer,
            self.user)
        sale2.action_confirm()
        self.assertEqual(len(sale2.picking_ids), 2)
        picking_stock_wh_03 = sale2.picking_ids.filtered(
            lambda pick: pick.location_id == self.warehouse_03.lot_stock_id)
        self.assertEqual(len(picking_stock_wh_03), 1)
        self.assertEqual(len(picking_stock_wh_03.move_ids), 1)
        self.assertEqual(picking_stock_wh_03.move_ids.product_uom_qty, 9)
        picking_stock_wh = sale2.picking_ids.filtered(
            lambda pick: pick.location_id == self.stock_wh.lot_stock_id)
        self.assertEqual(len(picking_stock_wh), 1)
        self.assertEqual(len(picking_stock_wh.move_ids), 1)
        self.assertEqual(picking_stock_wh.move_ids.product_uom_qty, 1)
    ...
    def test_cond_alternative_warehouse_virtual_qtyfield_without_stock_variant(
            self):
        self.main_company.product_qty_field = 'virtual_available'
        self.assertEqual(self.product_01.with_context(
            location=self.stock_wh.lot_stock_id.id).qty_available, 0)
        ...
        self.assertEqual(self.product_01.with_context(
            location=self.warehouse_02.lot_stock_id.id).qty_available, 0)
        self.assertEqual(self.product_01.with_context(
            location=self.warehouse_02.lot_stock_id.id).virtual_available, -1)
        sale2 = self.create_sale(
            self.product_01, 10, self.warehouse_by_condition, self.customer,
            self.user)
        sale2.action_confirm()
        self.assertEqual(len(sale2.picking_ids), 1)
        picking = sale2.picking_ids
        self.assertEqual(len(picking.move_ids), 1)
        self.assertEqual(picking.move_ids.product_id, self.product_01)
        self.assertEqual(
            picking.move_ids.location_id, self.stock_wh.lot_stock_id)
    ...
    def test_cond_alternative_wh_virtual_qtyfield_several_lines_without_stock(
            self):
        self.main_company.product_qty_field = 'virtual_available'
        self.assertEqual(self.product_01.with_context(
            location=self.stock_wh.lot_stock_id.id).qty_available, 0)
        ...
        self.assertEqual(picking.move_ids[0].product_id, self.product_01)
        self.assertEqual(picking.move_ids[1].product_id, self.product_02)
        self.assertEqual(
            picking.move_ids[0].location_id, self.warehouse_02.lot_stock_id)
        self.assertEqual(
            picking.move_ids[0].location_dest_id, self.customer_loc)
        self.assertEqual(
            picking.move_ids[1].location_id, self.warehouse_02.lot_stock_id)
        self.assertEqual(
            picking.move_ids[1].location_dest_id, self.customer_loc)
        self.assertEqual(
            picking.picking_type_id, self.warehouse_02.out_type_id)
        picking.action_assign()
        self.assertEqual(picking.state, 'confirmed')
    ...
    def test_cond_zip_and_alternative_whs_virtual_qtyfield_with_stock_variant(
            self):
        self.main_company.product_qty_field = 'virtual_available'
        self.assertEqual(self.product_01.with_context(
            location=self.stock_wh.lot_stock_id.id).qty_available, 0)
