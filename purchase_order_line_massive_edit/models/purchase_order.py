...
class PurchaseOrder(models.Model):
    ...
    def action_lines_massive_edit(self):
        self.ensure_one()
        wiz = self.env['wiz.purchase_order_line_edit'].create({
            'picking_id': (
                self.picking_ids and self.picking_ids.ids[0] or None)})
        ...
        view = self.env.ref(
            'purchase_order_line_massive_edit.wiz_purchase_order_line_edit')
        return {
