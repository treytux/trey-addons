...
class TestStockPickingModifyQtyDoneWizard(TransactionCase):
    ...
    def get_wizard_calling_default_get(self):
        wizard = self.env['stock.picking.modify_qty_done'].with_context(
    ...
    def check_new_wizard_lines_empty(self, wizard):
        ...
        self.assertEqual(line_product_1.reserved_uom_qty, 1)
        ...
        self.assertEqual(line_product_2.reserved_uom_qty, 2)
    ...
    def test_fill_lines_manually(self):
        ...
        move_product_1 = self.picking.move_ids.filtered(
            lambda m: m.product_id == self.product_1)
        self.assertEqual(move_product_1.quantity_done, 1)
        move_product_2 = self.picking.move_ids.filtered(
            lambda m: m.product_id == self.product_2)
        self.assertEqual(move_product_2.quantity_done, 10)
    ...
    def test_button_all_to_zero(self):
        ...
        move_product_1 = self.picking.move_ids.filtered(
            lambda m: m.product_id == self.product_1)
        self.assertEqual(move_product_1.quantity_done, 0)
        move_product_2 = self.picking.move_ids.filtered(
            lambda m: m.product_id == self.product_2)
        self.assertEqual(move_product_2.quantity_done, 0)
    ...
    def test_button_all_to_necessary(self):
        ...
        move_product_2 = self.picking.move_ids.filtered(
            lambda m: m.product_id == self.product_2)
        self.assertEqual(move_product_2.quantity_done, 2)
