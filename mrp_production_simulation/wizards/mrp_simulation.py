...
class WizMrpSimulation(models.TransientModel):
    ...
    def get_stock_location(self, location):
        return location.warehouse_id.lot_stock_id
        ...
        for child_bom_line in bom_line.child_line_ids:
            ...
            if model == 'mrp.production':
                ...
                mrp_stock_location = self.get_stock_location(
                    mrp.location_src_id)
                qty_available = (
                    child_bom_line.product_id.type == 'service' and 999.99
                    or child_bom_line.product_id.with_context(
                ...
                qty_available = (
                    child_bom_line.product_id.type == 'service' and 999.99
                    or child_bom_line.product_id.qty_available)
                virtual_available = (
                    child_bom_line.product_id.type == 'service' and 999.99
                    or child_bom_line.product_id.virtual_available)
            else:
                raise ValidationError(_(
                    'Model %s not contemplated!') % model._name)
            res.append({
    ...
    def get_product_components(self, obj, product):
        ...
        for bom_line in bom.bom_line_ids:
            product_parent_id = (
                product in bom_line.bom_id.product_id and product.id
                or product in
                bom_line.bom_id.product_tmpl_id.product_variant_ids
                and product.id)
            if not product_parent_id:
                raise ValidationError(_('Product parent is not found.'))
            level = 1
            qty_pending_produce, qty_pending_buy = self.get_qty_pending(
            ...
            if obj._name == 'mrp.production':
                ...
                mrp_stock_location = self.get_stock_location(
                    mrp.location_src_id)
                qty_available = (
                    bom_line.product_id.type == 'service' and 999.99
                    or bom_line.product_id.with_context(
                        location=mrp_stock_location.id).qty_available)
                virtual_available = (
                    bom_line.product_id.type == 'service' and 999.99
                    or bom_line.product_id.with_context(
                ...
                qty_available = (
                    bom_line.product_id.type == 'service' and 999.99
                    or bom_line.product_id.qty_available)
                virtual_available = (
                    bom_line.product_id.type == 'service' and 999.99
                    or bom_line.product_id.virtual_available)
            res.append({
    ...
    def get_qty2produce(self, res, qty_type):
        ...
        return int(min([
            ...
            if 'product_parent_id' in line[2]
            and line[2]['product_parent_id'] == res['main_product_id']]))
    ...
    def get_product2buy(self, simulation_lines):
        ...
        for line in simulation_lines:
            ...
            if line['product_id'] not in products2buy_group:
                ...
                products2buy_group[line['product_id']] += line[
                    'qty_pending_buy']
        for k, v in products2buy_group.items():
            products2buy.append({
                'product_id': k,
                'qty_pending_buy': v,
            })
    ...
    def get_product2produce(self, simulation_lines):
        ...
        for line in simulation_lines:
            ...
            if line['product_id'] not in products2produce_group:
                ...
                products2produce_group[line['product_id']] += line[
                    'qty_pending_produce']
        for k, v in products2produce_group.items():
    ...
    def default_get(self, fields):
        ...
        if active_model == 'mrp.production':
            ...
            raise ValidationError(_(
                'Model %s not contemplated!') % obj._name)
