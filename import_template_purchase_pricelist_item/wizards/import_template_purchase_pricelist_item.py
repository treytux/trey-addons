...
class ImportTemplatePurchasePricelistItem(models.TransientModel):
    _name = 'import.template.purchase.pricelist.item'
    ...
    def import_file(self, simulation=True):
        ...
        for index, row in df.iterrows():
            wizard.savepoint('import_template_purchase_pricelist_item')
            row_index = index + 2
            wizard.step(
            ...
            for error in errors:
                all_errors.append((row_index, [error]))
            if simulation:
                wizard.rollback('import_template_purchase_pricelist_item')
                continue
        ...
        for item_data in items2create:
            try:
                ...
                wizard.release('import_template_purchase_pricelist_item')
