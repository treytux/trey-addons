odoo.define('website_sale_product_grid.FancyBox', function (require) {
    'use strict';
    require('web.dom_ready')
    let Class = require('web.Class')
    let FancyBox = Class.extend({
        fancy_box_selector: '[data-fancybox="product_grid_gallery"]',
        init: function () {
...
odoo.define('website_sale_product_grid.CartQuantities', function (require) {
    'use strict';
    require('web.dom_ready')
    let Class = require('web.Class')
...
odoo.define('website_sale_product_grid.QuantityButton', function (require) {
    'use strict';
    require('web.dom_ready')
    let Class = require('web.Class')
    let QuantityButton = Class.extend({
        qty_btn_selector: 'a.js_wspg_qty_btn',
        init: function () {
            let self = this
...
odoo.define('website_sale_product_grid.GridButton', function (require) {
    'use strict';
    require('web.dom_ready')
    let Class = require('web.Class')
    let GridButton = Class.extend({
        grid_btn_selector: 'a.js_wspg_grid_btn',
        init: function () {
            let self = this
        ...
        set_value: function (set_attribute) {
            let $inputs = $('input.js_wspg_variant_qty')
...
odoo.define('website_sale_product_grid.GridButtonRow', function (require) {
    'use strict';
    require('web.dom_ready')
    let Class = require('web.Class')
    let GridButtonRow = Class.extend({
        grid_btn_row_selector: 'a.js_wspg_grid_btn_row',
        init: function () {
            let self = this
        ...
        set_quantities_to: function (obj) {
            let $inputs = $(obj).closest('tr').find('input.js_wspg_variant_qty')
            $inputs.each(function(index){
...
odoo.define('website_sale_product_grid.AddToCart', function (require) {
    'use strict';
    require('web.dom_ready')
    let Class = require('web.Class')
...
odoo.define('website_sale_product_grid.SearchAndFilter', function (require) {
    'use strict';
    require('web.dom_ready')
    ...
    let SearchAndFilter = Class.extend({
        ...
        init: function () {
            let self = this
            let $discount_values = $('.js_wspg_cost_price .o_wsdb_badge_value')
            if($discount_values.length > 0) {
                let $grid_actions_right = $('.js_wspg_grid_actions_right')
                if($grid_actions_right.length > 0) {
                    let $discount_filters_buttons = $('<div class="js_wspg_discount_filters_buttons mb16"><div class="btn-group btn-group-toggle" data-toggle="buttons"></div></div>')
                    $discount_filters_buttons.hide()
                    let $discount_filters_buttons_group = $discount_filters_buttons.find('.btn-group')
                    $grid_actions_right.append($discount_filters_buttons)
                    let discount_values = $discount_values.map(function() {
                        return parseInt(this.innerHTML);
                    }).get()
                    discount_values.unshift(0)
        ...
        apply_search_and_filters: function () {
            let self = this
            let $grid_table_items = $('.js_wspg_grid_table_item')
            $grid_table_items.each(function(){
                if(self.search == '' || $(this).find('.js_wspg_variant_search').val().includes(self.search)){
                    $(this).show()
                } else {
                    $(this).hide()
                }
            ...
            $discount_values.each(function(){
                if((self.discount == 0 || parseInt(this.innerHTML) == self.discount) && (self.search == '' || $(this).closest('tr').find('.js_wspg_variant_search').val().includes(self.search))){
