...
class ImportTemplateSaleAmazon(models.TransientModel):
    ...
    def field_get_or_create(self, model, name, search_field, is_required):
        ...
        if not name:
            if is_required:
                errors.append(_('%s empty not found.') % model_name)
            return None, errors
    ...
    def partner_id_get_or_create(self, name):
        partner_id, errors = self.field_get_or_create(
            'res.partner', name, 'name', True)
        if partner_id and self.force_partner:
            partner = self.env['res.partner'].browse(partner_id)
            partner.parent_id = self.parent_partner_id.id
        return partner_id, errors
    ...
    def get_sale_data(self, df, index):
        ...
        data = {
            ...
            'partner_invoice_id': (
                self.force_partner and self.parent_partner_id.id
                or partner_id),
            'from_import_sale_amazon': True,
        }
        return data, errors
    ...
    def get_partner_data(self, df, index):
        ...
        data = {
            ...
            'zip': (
                df['zip'][index]
                and str(int(df['zip'][index])).zfill(5) or ''),
            'state_id': state_id,
            'country_id': country_id,
        }
        if not data['email']:
        ...
        for index, row in df.iterrows():
            ...
            if data_order.get('origin', None):
                amazon_sale_number = (
                    ...
                    and data_order['origin'].split(': ')[1] or '')
