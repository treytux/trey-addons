###############################################################################
#
#    Trey, Kilobytes de Soluciones
#    Copyright (C) 2018-Today Trey, Kilobytes de Soluciones <www.trey.es>
#
#    This program is free software: you can redistribute it and/or modify
#    it under the terms of the GNU Affero General Public License as
#    published by the Free Software Foundation, either version 3 of the
#    License, or (at your option) any later version.
#
#    This program is distributed in the hope that it will be useful,
#    but WITHOUT ANY WARRANTY; without even the implied warranty of
#    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
#    GNU Affero General Public License for more details.
#
#    You should have received a copy of the GNU Affero General Public License
#    along with this program.  If not, see <http://www.gnu.org/licenses/>.
#
###############################################################################
{
    'name': 'Vertical Spain',
    'summary': 'Dependencies addons for account spanish instance',
    'category': 'Vertical',
    'version': '12.0.1.13.0',
    'author': 'Trey (www.trey.es)',
    'website': 'https://www.trey.es',
    'license': 'AGPL-3',
    'depends': [
        'account_bank_statement_search_account',
        'account_banking_sepa_credit_transfer',
        'account_banking_sepa_direct_debit',
        'account_cancel',
        'account_chart_update',
        'account_due_list',
        'account_financial_report',
        'account_financial_report_order_move',
        'account_fiscal_year',
        'account_invoice_constraint_chronology',
        'account_invoice_outstanding_credits_debits_view_fix',
        'account_invoice_refund_line_selection',
        'account_invoice_simplified',
        'account_invoice_supplier_reference_year',
        'account_invoice_tax_required',
        'account_invoice_warnings',
        'account_lock_date_update',
        'account_move_line_from_partner',
        'account_move_line_tax_editable',
        'account_payment_mode',
        'account_payment_order',
        'account_payment_purchase',
        'account_payment_return',
        'account_payment_return_import',
        'account_payment_sale',
        'account_payment_show_invoice',
        'account_payment_term_extension',
        'account_renumber',
        'account_tag_menu',
        'account_tax_balance',
        'account_type_menu',
        'l10n_es',
        'l10n_es_account_asset',
        'l10n_es_account_bank_statement_import_n43',
        'l10n_es_account_banking_sepa_fsdd',
        'l10n_es_account_invoice_sequence',
        'l10n_es_aeat',
        'l10n_es_aeat_mod111',
        'l10n_es_aeat_mod115',
        'l10n_es_aeat_mod123',
        'l10n_es_aeat_mod216',
        'l10n_es_aeat_mod303',
        'l10n_es_aeat_mod303_extra_data',
        'l10n_es_aeat_mod347',
        'l10n_es_aeat_mod349',
        'l10n_es_aeat_mod390',
        'l10n_es_dua',
        'l10n_es_irnr',
        'l10n_es_mis_report',
        'l10n_es_partner',
        'l10n_es_toponyms',
        'l10n_es_vat_book',
        'l10n_es_vat_book_extra_data',
        'print_formats_l10n_es_aeat_mod347',
        'product_form_sale_link_fix',
        'sale_order_invoice_create_payment_mode',
    ],
    'application': True,
    'installable': True,
}
