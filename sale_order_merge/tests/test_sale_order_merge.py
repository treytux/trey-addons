...
class TestSaleOrderMerge(TransactionCase):
    ...
    def create_sale_order(
        ...
        partner_invoice = partner_invoice and partner_invoice or partner
        partner_shipping = partner_shipping and partner_shipping or partner
        ...
        order.order_line.product_id_change()
        return order
    ...
    def test_new_order_is_correct(self):
        ...
        aditional_line = self.sale_line_model.new({
            'order_id': sale_03.id,
            'product_id': self.product_01.id,
            'product_uom_qty': 1,
            'price_unit': 49.99,
        })
        aditional_line.product_id_change()
        aditional_line.create(
            aditional_line._convert_to_write(aditional_line._cache))
        wizard = self.wiz_model.with_context({
            'active_ids': [
                sale_01.id,
                sale_02.id,
        ...
        self.assertEquals(len(new_order.order_line), 4)
        self.assertEquals(new_order.origin, 'SO001, SO002, SO003')
        self.assertIn('RefA', new_order.client_order_ref)
        self.assertIn('RefB', new_order.client_order_ref)
        self.assertEquals(new_order.partner_id, self.partner_01)
        self.assertEquals(new_order.partner_invoice_id, self.partner_01)
        self.assertEquals(new_order.partner_shipping_id, self.partner_01)
        self.assertEquals(new_order.pricelist_id, self.pricelist_01)
        self.assertEquals(len(new_order.message_ids), 3)
        self.assertIn('sale order has been ', new_order.message_ids[0].body)
        self.assertEquals(sale_01.state, 'draft')
        self.assertIn('has been merged in', sale_01.message_ids[0].body)
        self.assertEquals(sale_02.state, 'draft')
        self.assertIn('has been merged in', sale_02.message_ids[0].body)
        self.assertEquals(sale_03.state, 'draft')
