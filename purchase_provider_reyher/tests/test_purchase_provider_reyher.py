...
class TestPurchaseProviderReyher(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.supplierinfo_01 = self.env['product.supplierinfo'].create({
            ...
            'product_code': '000975080000240000',
            'price': 25,
        })
        self.supplierinfo_02 = self.env['product.supplierinfo'].create({
            ...
            'partner_id': self.supplier.id,
            'product_code': '000975080000140000',
            'price': 35,
        })
    ...
    def test_connector_reyher_purchase_01(self):
        ...
        self.assertAlmostEqual(
            ...
            round(line_01.total_price_reyher / line_01.product_qty, 2), 0)
        self.assertAlmostEqual(
            purchase.order_line[1].price_unit,
            round(line_02.total_price_reyher / line_02.product_qty, 2), 0)
        self.assertEqual(len(purchase.message_ids), len(messages_01) + 1)
        self.assertIn(
            'Prices updated with supplier connector on',
            purchase.message_ids[0].preview)
    ...
    def test_cron_reyher_update_product_prices_without_products(self):
        with self.assertRaises(exceptions.ValidationError) as error:
            self.env['product.template'].cron_reyher_update_product_prices(
                domain=[('id', '=', 0)])
        self.assertEqual(
            error.exception.args[0], 'No products found with Reyher supplier')

    def test_job_reyher_update_price(self):
        self.check_credentials()
        self.supplierinfo_01.product_code = '000975080000240000'
        product_templates = (
        ...
        self.assertNotAlmostEqual(price_previous, self.supplierinfo_01.price)

    def test_job_reyher_update_price_mock(self):
        self.supplierinfo_01.product_code = '0009750800002400000001'
        self.supplierinfo_02.product_code = '00097508000014000000001'
        product_templates = (
            self.product_01.product_tmpl_id | self.product_02.product_tmpl_id)
        mock_path = (
