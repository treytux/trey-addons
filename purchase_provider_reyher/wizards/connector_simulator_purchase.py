...
class ConnectorSimulatorPurchase(models.TransientModel):
    _inherit = 'connector.simulator.purchase'
    ...
    def action_to_step_2(self):
        ...
        if not connector:
            return res
        if connector.supplier_id != self.order_id.partner_id:
            self.lines.write({
                'msg_reyher': _(
                    'Supplier connector not valid for order partner'),
            })
            return res
        if not connector.token_reyher or (
                fields.Date.today() > connector.token_validity_date_reyher):
            url_authenticate = 'https://rio.reyher.de/rest/reyher_int_de/V1/'\
        ...
        url_simulate = 'https://rio.reyher.de/rest/reyher_int_de/V1/'\
                       'sapordersimulate/mine/getOrdersimulate'
        headers = {
    ...
    def action_to_step_done(self):
        ...
        for line in self.lines.filtered(
            ...
            line.purchase_line_id.price_unit = line.price_unit_reyher
        msg = _('Prices updated with supplier connector on %s by %s') % (
...
class ConnectorSimulatorPurchaseLine(models.TransientModel):
    _inherit = 'connector.simulator.purchase.line'
    ...
    total_price_reyher = fields.Float(
        string='Total price including discounts and surcharges',
    )
    qty_available_reyher = fields.Float(
        string='Qty available Reyher',
    )
    ...
    def _compute_line_color(self):
        ...
        for line in self:
            if line.purchase_id.supplier_mode != 'reyher':
                continue
            if line.qty_available_reyher > line.product_qty:
                line.line_color = 'blue'
            elif line.qty_available_reyher == line.product_qty:
