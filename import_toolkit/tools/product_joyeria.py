...
class ProductCategory(struct.Csv):
    _from = 'raya_joyeria_product_category.txt'
    ...
    def to(self):
        for category in self.read():  # (count=200):
            try:
                ...
                _log.error('Product Category data %s' % category.dict())
                _log.error('With errors: %s' % e)
                raise
            _log.info('Product Category "%s" imported with id %s' % (
                category.name, c.id))


class ProductBrand(struct.Csv):
    _from = 'raya_joyeria_product_brand.txt'
    _first_row_titles = True
    _initialized = False
    ...
    def to(self):
        for brand in self.read():  # (count=200):
            try:
                ...
                _log.error('Product Brand data %s' % brand.dict())
                _log.error('With errors: %s' % e)
                raise
            _log.info('Product Brand "%s" imported with id %s' % (
                brand.name, b.id))


class ProductTemplate(struct.Csv):
    _from = 'raya_joyeria_product.txt'
    _first_row_titles = True
    _initialized = False
    ...
    _map = [
        ...
        parser.Custom(src='available_in_pos', dst='available_in_pos',
                      parser='parser_boolean'),
        parser.Str(src='description', dst='description'),
    ...
    def to(self):
        for product in self.read():  # (count=200):
            try:
                ...
                _log.error('Product Template data %s' % product.dict())
                _log.error('With errors: %s' % e)
                raise
            _log.info('Product Template "%s" imported with id %s' % (
                product.name, p.id))
