...
class TestAccountMoveImportA3nom(TransactionCase):
    ...
    def test_access_account(self):
        ...
        self.assertEquals(len(moves.line_ids), 5)
        move_line_465 = moves.line_ids.filtered(
            lambda ln: '465' in ln.account_id.code)
        self.assertEquals(move_line_465.partner_id, self.env.user.partner_id)
        self.assertEquals(move_line_465.account_id, account_465)
        self.assertEquals(move_line_465.credit, 1252.58)
        move_line_640 = moves.line_ids.filtered(
            lambda ln: '640' in ln.account_id.code)
        self.assertEquals(move_line_640.partner_id, self.env.user.partner_id)
        self.assertEquals(move_line_640.account_id, account_640)
        self.assertEquals(move_line_640.debit, 1409.93)
        ...
        self.assertEquals(move_line_4751.partner_id, self.env.user.partner_id)
        self.assertEquals(move_line_4751.account_id, account_4751)
        self.assertEquals(move_line_4751.credit, 66.13)
        ...
        self.assertEquals(move_line_642.partner_id, self.env.user.partner_id)
        self.assertEquals(move_line_642.account_id, account_642)
        self.assertEquals(move_line_642.debit, 450.90)
        move_line_476 = moves.line_ids.filtered(
            lambda ln: '476' in ln.account_id.code)
        self.assertEquals(move_line_476.partner_id, self.env.user.partner_id)
        self.assertEquals(move_line_476.account_id, account_476)
        self.assertEquals(move_line_476.credit, 542.12)
