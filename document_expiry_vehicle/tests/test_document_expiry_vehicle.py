...
class TestDocumentExpiryVehicle(TransactionCase):
    ...
    def test_document_expiry_vehicle_activity(self):
        activity_type = self.env.ref(
            'document_expiry_vehicle.activity_document_expired_warn_vehicle')
        vehicle_activities = self.vehicle.activity_ids.filtered(
            lambda activity: activity.activity_type_id == activity_type)
        self.assertTrue(vehicle_activities)
        self.assertTrue(all(
            activity.res_id == self.vehicle.id
            for activity in vehicle_activities))
        self.assertTrue(any(
            'documentation in state' in activity.summary
            for activity in vehicle_activities))
