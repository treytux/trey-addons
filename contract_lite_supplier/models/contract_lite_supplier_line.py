...
class ContractLiteSupplierLine(models.Model):
    _name = 'contract_lite_supplier.line'
    ...
    def _compute_price_unit(self):
        for line in self:
            ...
            if line.automatic_price and line.product_id:
                line.price_unit = line.get_vendor_price_unit()
            else:
    ...
    def _compute_price_subtotal(self):
        for line in self:
            discount = max(line.discount or 0.0, 0.0)
            line.price_subtotal = (
                (line.quantity or 0.0)
                * (line.price_unit or 0.0)
                * (1.0 - (discount / 100.0)))

    @api.depends('recurring_next_date', 'recurring_interval',
                 'recurring_rule_type')
    ...
    def get_vendor_price_unit(self):
        self.ensure_one()
    ...
    def render_invoice_line_description(self, date_start):
        ...
        return self.contract_id.render_line_description(
            template, date_start, date_end) or (
