...
class StockBarcodesRelocateLocation(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        lines = self.env[
            'stock.barcodes.relocate.location.line'
        ].search([
            ('wizard_id', '!=', active_id),
    ...
    def get_all_products_from_location(self):
    ...
    def show_picking_lines(self):
        if not self.location_id:
            raise ValidationError(_(
                'No location origin asigned'))
        if not self.location_dest_id:
            raise ValidationError(_(
                'No destination location asigned'))
        if self.location_id == self.location_dest_id:
            raise ValidationError(_(
                'Source and destination location are the same'))
        self.get_all_products_from_location()
        self.step = 1
    ...
    def process_barcode(self, barcode):
        ...
        if len(location) == 1:
            ...
            raise ValidationError(_(
                'More than one location found %s') % (barcode))
        else:
            raise ValidationError(_(
                'No location found with this barcode %s') % (barcode))
        return
    ...
    def validate_picking(self):
        if not self.check_quants_location():
            raise ValidationError(_(
                'Error in the origin location of some line of the picking'))
        if len(self.line_ids) == 0:
            raise ValidationError(_(
                'No lines to create picking to relocate complete location'))
        if not self.location_id:
            raise ValidationError(_(
                'No location origin asigned'))
        if not self.location_dest_id:
