...
class SaleSessionValidate(models.TransientModel):
    ...
    def action_confirm(self):
        ...
        if self.session_id.mismatch_open_move_ids:
            self.session_id.mismatch_open_move_ids.action_post()
        if self.session_id.mismatch_close_move_ids:
        ...
        cash_journals = self.session_id.payment_ids.filtered(
            lambda p: p.journal_id.type == 'cash').mapped('journal_id')
        ...
        credit_vals.update({
            ...
            'account_id': cash_journals.default_credit_account_id.id,
        })
