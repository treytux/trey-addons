...
class CrmTeam(models.Model):
    ...
    send_email_on_close = fields.Boolean(
        string='Send email on sale session close',
    )
    notified_partners_on_close = fields.Many2many(
        comodel_name='res.partner',
        string='Notified partners on sale session close',
    )
    autocomplete_amount = fields.Boolean(
        string='Autocomplete amount',
        help='Autocomplete amount paid',
    )
    sale_session_sequence_id = fields.Many2one(
        comodel_name='ir.sequence',
        string='Sale session sequence',
        help='Sequence for sale sessions of this team',
    )
    ...
    def get_actual_total_cash(self, journal_id=False):
        ...
        for account in cash_journals.mapped('default_account_id'):
