    ...
    def _import_file_payroll_comeralia(self, journal, content):
        ...
        for line in [ln for ln in content.split(return_line) if ln.strip()]:
            ...
            if move_type == 'NOM':
                ...
                task = employee.comeralia_project_task_id
                analytic_account_id = (
                    task.project_id.analytic_account_id.id
                    if task and task.project_id and task.project_id.analytic_account_id
                    else False
                )
                if data['amount'] < 0:
                    data['amount'] *= -1
            ...
            move_line.append({
                ...
                'analytic_distribution': {analytic_account_id: 100.0} if
                ...
                'tax_repartition_line_id': (
                    ...
                    if account.code.startswith('4751') else False
                ),
            })
