...
class TestAccountMoveImportComeralia(TransactionCase):
    ...
    def setUp(self):
        ...
        ).try_loading(company=self.env.user.company_id)
    ...
    def test_access_account(self):
        ...
        self.assertEqual(len(moves[0].line_ids), 4)
        move_line_640 = moves[0].line_ids.filtered(
            lambda ln: '640' in ln.account_id.code)
        self.assertEqual(move_line_640.partner_id, user_1.partner_id)
        self.assertEqual(move_line_640.account_id, account_640)
        self.assertEqual(move_line_640.debit, 7238.80)
        ...
        self.assertEqual(move_line_465.partner_id, user_1.partner_id)
        self.assertEqual(move_line_465.account_id, account_465)
        self.assertEqual(move_line_465.credit, 3806.81)
        self.assertEqual(len(move_line_465.tax_ids), 0)
        self.assertFalse(move_line_465.tax_line_id)
        move_line_476 = moves[0].line_ids.filtered(
            lambda ln: '476' in ln.account_id.code)
        self.assertEqual(move_line_476.partner_id, user_1.partner_id)
        self.assertEqual(move_line_476.account_id, account_476)
        self.assertEqual(move_line_476.credit, 1477.51)
        self.assertEqual(len(move_line_476.tax_ids), 0)
        self.assertFalse(move_line_476.tax_line_id)
        move_line_4751 = moves[0].line_ids.filtered(
            lambda ln: '4751' in ln.account_id.code)
        self.assertEqual(move_line_4751.partner_id, user_1.partner_id)
        self.assertEqual(move_line_4751.account_id, account_4751)
        self.assertEqual(move_line_4751.credit, 1954.48)
        ...
        move_line_640 = moves[1].line_ids.filtered(
            lambda ln: '640' in ln.account_id.code)
        self.assertEqual(move_line_640.partner_id, user_2.partner_id)
        self.assertEqual(move_line_640.account_id, account_640)
        self.assertEqual(move_line_640.debit, 3704.71)
        self.assertEqual(len(move_line_640.tax_ids), 1)
        self.assertFalse(move_line_640.tax_line_id)
        move_line_465 = moves[1].line_ids.filtered(
            lambda ln: '465' in ln.account_id.code)
        self.assertEqual(move_line_465.partner_id, user_2.partner_id)
        self.assertEqual(move_line_465.account_id, account_465)
        self.assertEqual(move_line_465.credit, 2749.02)
        self.assertEqual(len(move_line_465.tax_ids), 0)
        self.assertFalse(move_line_465.tax_line_id)
        move_lines_476 = moves[1].line_ids.filtered(
            lambda ln: '476' in ln.account_id.code)
        self.assertEqual(len(move_lines_476), 2)
        move_line_476 = move_lines_476[0]
        self.assertEqual(move_line_476.partner_id, user_2.partner_id)
        self.assertEqual(move_line_476.account_id, account_476)
        self.assertEqual(move_line_476.credit, 280.20)
        self.assertEqual(len(move_line_476.tax_ids), 0)
        self.assertFalse(move_line_476.tax_line_id)
        move_line_476 = move_lines_476[1]
        self.assertEqual(move_line_476.partner_id, user_2.partner_id)
        self.assertEqual(move_line_476.account_id, account_476)
        self.assertEqual(move_line_476.credit, 8.64)
        self.assertEqual(len(move_line_476.tax_ids), 0)
        self.assertFalse(move_line_476.tax_line_id)
    ...
    def test_access_account_mapped_and_iprf(self):
        ...
        self.assertEqual(len(moves[0].line_ids), 6)
        move_line_640 = moves[0].line_ids.filtered(
            lambda ln: '640' in ln.account_id.code)
        self.assertEqual(move_line_640.partner_id, user_1.partner_id)
        self.assertEqual(move_line_640.account_id, account_640_2)
        self.assertEqual(move_line_640.debit, 421.74)
        self.assertEqual(len(move_line_640.tax_ids), 1)
        self.assertFalse(move_line_640.tax_line_id)
        move_line_465 = moves[0].line_ids.filtered(
            lambda ln: '465' in ln.account_id.code)
        self.assertEqual(move_line_465.partner_id, user_1.partner_id)
        self.assertEqual(move_line_465.account_id, account_465)
        self.assertAlmostEqual(move_line_465.credit, 2539.24, places=2)
        self.assertEqual(len(move_line_465.tax_ids), 0)
        self.assertFalse(move_line_465.tax_line_id)
        move_lines_476 = moves[0].line_ids.filtered(
            lambda ln: '476' in ln.account_id.code)
        self.assertEqual(len(move_lines_476), 3)
        move_line_476 = move_lines_476[0]
        self.assertEqual(move_line_476.partner_id, user_1.partner_id)
        self.assertEqual(move_line_476.account_id, account_476)
        self.assertEqual(move_line_476.debit, 3000.3)
        self.assertEqual(len(move_line_476.tax_ids), 1)
        self.assertFalse(move_line_476.tax_line_id)
        move_line_476 = move_lines_476[1]
        self.assertEqual(move_line_476.partner_id, user_1.partner_id)
        self.assertEqual(move_line_476.account_id, account_476)
        self.assertEqual(move_line_476.credit, 258.83)
        self.assertEqual(len(move_line_476.tax_ids), 0)
        self.assertFalse(move_line_476.tax_line_id)
        move_line_476 = move_lines_476[2]
        self.assertEqual(move_line_476.partner_id, user_1.partner_id)
        self.assertEqual(move_line_476.account_id, account_476)
        self.assertEqual(move_line_476.credit, 8)
        self.assertEqual(len(move_line_476.tax_ids), 0)
        self.assertFalse(move_line_476.tax_line_id)
        move_line_4751 = moves[0].line_ids.filtered(
            lambda ln: '4751' in ln.account_id.code)
        self.assertEqual(move_line_4751.partner_id, user_1.partner_id)
        self.assertEqual(move_line_4751.account_id, account_4751)
        self.assertEqual(move_line_4751.credit, 615.97)
        self.assertEqual(len(move_line_4751.tax_ids), 0)
        self.assertEqual(irpf_tax, move_line_4751.tax_line_id)
        company_move = moves.filtered(
            lambda m: m.partner_id == self.env.user.company_id.partner_id)
    ...
    def test_access_account_mapped_and_advance(self):
        ...
        moves = wizard._import_file()
        self.assertEqual(len(moves), 2)
        self.assertEqual(len(moves[0].line_ids), 3)
        move_line_460 = moves[0].line_ids.filtered(
            lambda ln: '460' in ln.account_id.code)
        self.assertEqual(move_line_460.partner_id, user_1.partner_id)
        self.assertEqual(move_line_460.account_id, account_460)
        self.assertEqual(move_line_460.debit, 46.81)
        self.assertEqual(len(move_line_460.tax_ids), 1)
        self.assertFalse(move_line_460.tax_line_id)
        move_line_465 = moves[0].line_ids.filtered(
            lambda ln: '465' in ln.account_id.code)
        self.assertEqual(move_line_465.partner_id, user_1.partner_id)
        self.assertEqual(move_line_465.account_id, account_465)
        self.assertEqual(move_line_465.credit, 40)
        self.assertEqual(len(move_line_465.tax_ids), 0)
        self.assertFalse(move_line_465.tax_line_id)
        move_line_476 = moves[0].line_ids.filtered(
            lambda ln: '476' in ln.account_id.code)
        self.assertEqual(move_line_476.partner_id, user_1.partner_id)
        self.assertEqual(move_line_476.account_id, account_476)
        self.assertAlmostEqual(move_line_476.credit, 6.81, places=2)
        self.assertEqual(len(move_line_476.tax_ids), 0)
        self.assertFalse(move_line_476.tax_line_id)
        company_move = moves.filtered(
            lambda m: m.partner_id == self.env.user.company_id.partner_id)
