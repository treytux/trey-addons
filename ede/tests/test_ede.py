...
class TestEde(TransactionCase):
    ...
    def setUp(self):
        ...
        # vez, comentar los "self.env.cr.commit" del código para evitar que
        ...
        data = {
            ...
            'ede_test_wsdl':
                'https://webservices-test.ede.de:9443/ibis/ws/WS_EXT_ELC?wsdl',
        ...
        self.product_01 = self.env['product.product'].create({
            ...
            'list_price': 6.08,
            'seller_ids': [(0, 0, {
                'partner_id': self.ede_supplier.id,
            })],
        })
    ...
    def test_ede_workflow_from_button_ok(self):
        ...
        self.assertEqual(so.order_line[0].state, 'sale')
        simu_so = self.create_simulation_sale(so)
        self.assertTrue(simu_so)
        self.assertEqual(simu_so.state, 'step_1')
        simu_so.action_to_step_2()
        self.assertEqual(simu_so.state, 'step_2')
        ...
        self.assertEqual(simu_po.state, 'step_1')
        simu_po.action_to_step_2()
        self.assertEqual(simu_po.state, 'step_2')
        ...
        self.assertEqual(po.ede_state, 'C')
    ...
    def test_ede_workflow_from_button_error_authorization(self):
        ...
        self.assertEqual(len(po_log_lines), 1)
        self.assertEqual(po_log_lines.state, 'fail')
    ...
    def test_ede_workflow_from_cron_error_authorization(self):
        ...
        self.assertEqual(len(po_log_lines), 1)
        self.assertEqual(po_log_lines.state, 'fail')
        self.assertEqual(po_log_lines.ede_purchase_order_number, '5523022986')
        self.assertIn(
