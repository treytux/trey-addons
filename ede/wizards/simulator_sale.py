            ...
            if line.is_cost_changed:
                ...
                if line.supplierinfo:
                    ...
                    self.env['product.supplierinfo'].sudo().create({
                        ...
                        'partner_id': (
                            self.order_id.company_id.ede_supplier_id.id),
...
class SimulatorSaleLine(models.TransientModel):
    ...
    def _compute_supplierinfo(self):
        for line in self:
            supplier = line.sale_id.company_id.ede_supplier_id
            supplier_infos = (
                line.sale_line_id.product_id.product_tmpl_id.mapped(
                    'seller_ids').filtered(
    ...
    def _compute_line_color(self):
        for line in self:
            try:
                if line.ede_quantity_available >= line.product_uom_qty:
