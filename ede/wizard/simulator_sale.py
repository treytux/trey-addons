...
class SimulatorSale(models.TransientModel):
    ...
    def action_to_step_done(self):
        if not self.lines:
            raise exceptions.Warning(_('EDE not Return Simulation Products'))
        sale_order = self.lines[0].sale_line_id.order_id
        for line in self.lines:
            ...
            if line.is_cost_changed:
                if line.discount:
                    margin_sale = (1 - line.ede_cost_price / (
                        line.price_unit * (1 - (line.discount / 100))))*100
                else:
                    margin_sale = (1 - (
                        line.ede_cost_price / line.price_unit)) * 100
                margin_product = (1 - (
                    line.ede_cost_price / line.product_id.lst_price)) * 100
                data['margin'] = margin_sale
                data['standard_price'] = line.ede_cost_price
                data['is_ede_danger'] = line.is_ede_danger
                line.sale_line_id.product_id.write({
                    'supplier_price': line.ede_cost_price,
                    'margin': margin_product,
                    'standard_price': line.ede_cost_price,
                })
...
class SimulatorSaleLine(models.TransientModel):
    ...
    cost_price = fields.Float(
        related='sale_line_id.standard_price',
        string='Odoo Cost',
