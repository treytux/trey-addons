...
class PurchaseOrder(models.Model):
    ...
    def ede_check_status(self, orders):
        ...
        _log.info('=' * 75)
        for order in orders:
            if order.adarra_ede_state in ('email', 'done'):
                order.ede_state = 'C'
                continue
            ...
            for item in items:
                ...
                if not purchase_line:
                    if len(adarra_code) < 12:
                        purchase_line = order.mapped('order_line').filtered(
                            lambda l: l.product_id.default_code == adarra_code)
                    else:
                        msg_item = _(
    ...
    def ede_confirm_purchase_order(self, order=None, lines=None):
        ...
        if not pickings:
            if order.adarra_ede_state != 'purchase':
                return False
            else:
                return True
    ...
    def ede_confirm_sale_order(self, order=None, lines=None):
        ...
        if not order.origin or order.origin == '':
        ...
        pickings = sale_order.mapped('picking_ids').filtered(
            ...
            p.state in ('assigned', 'partially_available', 'waiting',
                        'confirmed') and p.picking_type_code == 'outgoing')
        if not pickings and order.adarra_ede_state == 'sale':
            return True
        if not pickings:
            return False
        for picking in pickings:
            if picking.state in ('waiting', 'partially_available'):
            ...
            for line in lines:
                move = self.env['stock.move'].search(
                    [('picking_id', '=', picking.id),
                     ...
                     ('product_id', '=', line[
                         'purchase_line'].product_id.id)], limit=1)
                if not move:
                    continue
                qty_delivered = float(
    ...
    def _run_ede_check_status(self):
        orders = self.env['purchase.order'].search([
            ...
            ('customer_shipping_id', '!=', None)], order='date_order desc')
        if not orders:
            return False
