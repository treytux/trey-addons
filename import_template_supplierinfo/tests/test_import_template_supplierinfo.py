...
class TestImportTemplateSupplierInfo(TransactionCase):
    ...
    def setUp(self):
        ...
        self.product_04_white = (
            self.product_tmpl_04.product_variant_ids.filtered(
                lambda p: (
                    p.product_template_attribute_value_ids.name == 'White')))
        self.product_04_black = (
            self.product_tmpl_04.product_variant_ids.filtered(
                lambda p: (
                    p.product_template_attribute_value_ids.name == 'Black')))
    ...
    def test_import_create_supplierinfo_ok(self):
        ...
        self.assertEqual(len(self.product_01.seller_ids), 0)
        wizard.action_import_from_simulation()
        self.assertEqual(wizard.total_rows, 3)
        self.assertEqual(len(wizard.line_ids), 0)
        self.assertEqual(wizard.total_warn, 0)
        self.assertEqual(wizard.total_error, 0)
        self.assertEqual(len(self.product_01.seller_ids), 1)
        supplierinfo_sup_01 = self.product_01.seller_ids.filtered(
            lambda s: s.partner_id.id == self.supplier_01.id)
        self.assertEqual(len(supplierinfo_sup_01), 1)
        self.assertEqual(supplierinfo_sup_01.product_name, 'supplier_name_01')
        self.assertEqual(supplierinfo_sup_01.product_code, 'supplier_code_01')
        self.assertEqual(supplierinfo_sup_01.min_qty, 10)
        self.assertEqual(supplierinfo_sup_01.price, 11.11)
        self.assertEqual(len(self.product_02.seller_ids), 1)
        supplierinfo_sup_02 = self.product_02.seller_ids.filtered(
            lambda s: s.partner_id.id == self.supplier_02.id)
        self.assertEqual(len(supplierinfo_sup_02), 1)
        self.assertEqual(supplierinfo_sup_02.product_name, 'supplier_name_02')
        self.assertEqual(supplierinfo_sup_02.product_code, 'supplier_code_02')
        self.assertEqual(supplierinfo_sup_02.min_qty, 5)
        self.assertEqual(supplierinfo_sup_02.price, 22.22)
        self.assertEqual(len(self.product_03.seller_ids), 1)
        supplierinfo_sup_03 = self.product_03.seller_ids.filtered(
            lambda s: s.partner_id.id == self.supplier_03.id)
        self.assertEqual(len(supplierinfo_sup_03), 1)
        self.assertEqual(supplierinfo_sup_03.product_name, 'supplier_name_03')
        self.assertEqual(supplierinfo_sup_03.product_code, 'supplier_code_03')
        self.assertEqual(supplierinfo_sup_03.min_qty, 15)
        self.assertEqual(supplierinfo_sup_03.price, 33.33)
    ...
    def test_import_write_supplierinfo_without_product_id_ok(self):
        ...
        self.assertEqual(self.product_01.seller_ids.product_code, 'SUP_CODE')
        wizard.action_import_from_simulation()
        self.assertEqual(wizard.total_rows, 3)
        self.assertEqual(len(wizard.line_ids), 0)
        self.assertEqual(wizard.total_warn, 0)
    ...
    def test_import_error_product_not_exists(self):
        ...
        self.assertEqual(len(self.product_01.seller_ids), 0)
        self.assertEqual(len(self.product_02.seller_ids), 0)
        wizard.action_import_from_simulation()
        self.assertEqual(wizard.total_rows, 2)
        self.assertEqual(len(wizard.line_ids), 1)
        self.assertEqual(wizard.total_warn, 0)
    ...
    def test_import_error_several_product(self):
        ...
        self.assertIn(_(
            ...
            wizard.line_ids[1].name)
        self.assertEqual(len(self.product_01.seller_ids), 0)
        self.assertEqual(len(self.product_03.seller_ids), 0)
        self.assertEqual(len(self.product_02.seller_ids), 1)
        supplierinfo_sup_02 = self.product_02.seller_ids.filtered(
            lambda s: s.partner_id.id == self.supplier_02.id)
        self.assertEqual(len(supplierinfo_sup_02), 1)
    ...
    def test_import_error_empty_supplier_ref(self):
        ...
        self.assertIn(_(
            '2: The \'partner_id\' field is required, you must fill it with a valid '
    ...
    def test_import_error_empty_values(self):
        ...
        supplierinfo_sup_01 = self.product_01.seller_ids.filtered(
            lambda s: s.partner_id.id == self.supplier_01.id)
        self.assertEqual(len(supplierinfo_sup_01), 1)
        self.assertEqual(supplierinfo_sup_01.product_name, '')
        self.assertEqual(supplierinfo_sup_01.product_code, '')
        self.assertEqual(supplierinfo_sup_01.min_qty, 0)
        self.assertEqual(supplierinfo_sup_01.price, 0)
        self.assertEqual(len(self.product_02.seller_ids), 1)
        supplierinfo_sup_02 = self.product_02.seller_ids.filtered(
            lambda s: s.partner_id.id == self.supplier_02.id)
        self.assertEqual(len(supplierinfo_sup_02), 1)
    ...
    def test_new_fields_ok(self):
        ...
        self.assertEqual(supplierinfo_sup_01.min_qty, 10)
        self.assertEqual(supplierinfo_sup_01.price, 11.11)
        self.assertEqual(supplierinfo_sup_01.delay, 7)
        self.assertEqual(len(self.product_02.seller_ids), 1)
        supplierinfo_sup_02 = self.product_02.seller_ids.filtered(
            lambda s: s.partner_id.id == self.supplier_02.id)
        self.assertEqual(len(supplierinfo_sup_02), 1)
        ...
        self.assertEqual(supplierinfo_sup_02.min_qty, 5)
        self.assertEqual(supplierinfo_sup_02.price, 22.22)
        self.assertEqual(supplierinfo_sup_02.delay, 14)
    ...
    def test_import_create_variants_ok(self):
        ...
        self.assertEqual(wizard.total_warn, 0)
        self.assertEqual(wizard.total_error, 0)
        self.assertEqual(len(self.product_tmpl_04.seller_ids), 0)
        self.assertEqual(len(self.product_04_black.seller_ids), 0)
        self.assertEqual(len(self.product_04_white.seller_ids), 0)
        wizard.action_import_from_simulation()
        self.assertEqual(wizard.total_rows, 2)
        self.assertEqual(len(wizard.line_ids), 0)
        self.assertEqual(wizard.total_warn, 0)
        self.assertEqual(wizard.total_error, 0)
        self.assertEqual(len(self.product_tmpl_04.seller_ids), 2)
        supplierinfo_sup_04_black = self.product_04_black.seller_ids.filtered(
            lambda s: s.partner_id == self.supplier_01
            and s.product_tmpl_id == self.product_tmpl_04
            and s.product_id == self.product_04_black)
        self.assertEqual(len(supplierinfo_sup_04_black), 1)
        self.assertEqual(
            supplierinfo_sup_04_black.product_name, 'supplier_name_01_black')
        self.assertEqual(
            supplierinfo_sup_04_black.product_code, 'supplier_code_01B')
        self.assertEqual(supplierinfo_sup_04_black.min_qty, 10)
        self.assertEqual(supplierinfo_sup_04_black.price, 11.11)
        supplierinfo_sup_04_white = self.product_04_white.seller_ids.filtered(
            lambda s: s.partner_id == self.supplier_01
            and s.product_tmpl_id == self.product_tmpl_04
            and s.product_id == self.product_04_white)
        self.assertEqual(len(supplierinfo_sup_04_white), 1)
        self.assertEqual(
            supplierinfo_sup_04_white.product_name, 'supplier_name_01_white')
        self.assertEqual(
            supplierinfo_sup_04_white.product_code, 'supplier_code_01W')
        self.assertEqual(supplierinfo_sup_04_white.min_qty, 10)
        self.assertEqual(supplierinfo_sup_04_white.price, 22.22)
    ...
    def test_import_write_variants_supplierinfo_without_product_id_ok(self):
        ...
        self.assertEqual(wizard.total_warn, 0)
        self.assertEqual(wizard.total_error, 0)
        self.assertEqual(len(self.product_tmpl_04.seller_ids), 1)
        wizard.action_import_from_simulation()
        self.assertEqual(wizard.total_rows, 2)
        self.assertEqual(len(wizard.line_ids), 0)
        self.assertEqual(wizard.total_warn, 0)
        self.assertEqual(wizard.total_error, 0)
        self.assertEqual(len(self.product_tmpl_04.seller_ids), 3)
        supplierinfo_sup_04 = self.product_tmpl_04.seller_ids.filtered(
            lambda s: s.partner_id == self.supplier_01
            and s.product_tmpl_id == self.product_tmpl_04
            and not s.product_id)
        self.assertEqual(len(supplierinfo_sup_04), 1)
        self.assertEqual(supplierinfo_sup_04.product_name, 'Supplier name')
        self.assertEqual(supplierinfo_sup_04.product_code, 'SUP_CODE')
        self.assertEqual(supplierinfo_sup_04.min_qty, 12)
        self.assertEqual(supplierinfo_sup_04.price, 0)
        supplierinfo_sup_04_black = self.product_04_black.seller_ids.filtered(
            lambda s: s.partner_id == self.supplier_01
            and s.product_tmpl_id == self.product_tmpl_04
            and s.product_id == self.product_04_black)
