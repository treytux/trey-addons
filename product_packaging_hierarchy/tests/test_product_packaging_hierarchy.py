...
class TestProductPackagingHierarchy(TransactionCase):
    ...
    def test_put_in_pack_hierarchy_with_parent_package(self):
        ...
        move_line_pack_1_product_01 = (
            self.picking.mapped('move_lines.move_line_ids').filtered(
                lambda ln: ln.product_id == self.product_01
                and ln.qty_done == 1))
        self.assertEquals(len(move_line_pack_1_product_01), 1)
        pack_1 = move_line_pack_1_product_01.result_package_id
        self.assertEquals(len(pack_1), 1)
        self.assertEquals(pack_1.parent_id, self.parent_package_01)
        self.assertEquals(self.parent_package_01.child_ids, pack_1)
        self.assertEquals(len(self.picking.package_level_ids), 1)
        move_line_pack_1_product_02 = (
            self.picking.mapped('move_lines.move_line_ids').filtered(
                lambda ln: ln.product_id == self.product_02
                and ln.qty_done == 1))
        self.assertEquals(len(move_line_pack_1_product_02), 1)
        self.assertEquals(
        ...
        self.assertEquals(len(move_line_pack_2_product_01), 1)
        move_line_pack_2_product_02 = (
            self.picking.mapped('move_lines.move_line_ids').filtered(
                lambda ln: ln.product_id == self.product_02
                and ln.result_package_id != pack_1))
        self.assertEquals(len(move_line_pack_2_product_02), 1)
        self.assertEquals(
        ...
        self.assertEquals(len(pack_2), 1)
        self.assertEquals(pack_2.parent_id, self.parent_package_02)
        self.assertEquals(self.parent_package_02.child_ids, pack_2)
        self.assertEquals(len(self.picking.package_level_ids), 2)
        self.picking.action_done()
        self.assertEquals(self.picking.state, 'done')
        self.assertEqual(len(pack_1.quant_ids), 2)
        self.assertEqual(pack_1.quant_ids.mapped('quantity'), [1, 1])
        self.assertEqual(len(pack_2.quant_ids), 2)
        self.assertEqual(pack_2.quant_ids.mapped('quantity'), [9, 1])
