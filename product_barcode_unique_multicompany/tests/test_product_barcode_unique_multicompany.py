...
class TestProductBarcodeUniqueMulticompany(TransactionCase):
    ...
    def test_product_barcode_unique_multicompany(self):
        ...
        self.assertIn(
            'The barcode 1234567890 already exists in another product for '
            'this company. It must be unique!',
            result.exception.args[0])

    def test_product_barcode_unique_only_one_company(self):
    ...
    def test_product_barcode_unique_only_without_company(self):
        self.new_user.company_ids = [(6, 0, [self.main_company.id])],
        self.env['product.product'].sudo(
            self.new_user.id).create({
                'type': 'service',
